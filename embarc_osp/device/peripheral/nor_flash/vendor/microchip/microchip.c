...
static int32_t microchip_param_read(dw_ssi_t *dw_ssi);
static int32_t microchip_quad_entry(dw_ssi_t *dw_ssi);
static int32_t microchip_quad_exit(dw_ssi_t *dw_ssi);
static int32_t microchip_qpi_entry(dw_ssi_t *dw_ssi);
static int32_t microchip_qpi_exit(dw_ssi_t *dw_ssi);
static int32_t microchip_status_read(dw_ssi_t *dw_ssi, uint32_t status);

static flash_dev_ops_t microchip_ops = {
	.quad_entry = microchip_quad_entry,
	.quad_exit = microchip_quad_exit,
	.qpi_entry = microchip_qpi_entry,
	.qpi_exit = microchip_qpi_exit,

	.status = microchip_status_read
};

#ifdef STATIC_EXT_FLASH_PARAM
static flash_device_t microchip_dev = {
...
#ifdef STATIC_EXT_FLASH_PARAM
	ext_dev = &microchip_dev;
#else
	if (0 == ext_dev_open_flag) {
		result = microchip_param_read(dw_ssi);
		if (E_OK == result) {
			ext_dev = &microchip_dev;
			ext_dev->ops = &microchip_ops;
		}
	} else {
		ext_dev = &microchip_dev;
	}
...
#if 0
int32_t microchip_status_read_test(dw_ssi_t *dw_ssi)
{
	int32_t result = E_OK;
