...
/* radio power detector command */
static BaseType_t radioPDTCommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString);

#if TIA_SAT ==1
/* radio tia saturation detector */
static BaseType_t radioTiaSatCommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString);
#endif
 ...
 * parameter : <mode_sel> <ch_index> - select tx auto channel index and mode
 *
 * note      : auto tx mode selection
 ...
 *             5: down state
 *             6: idle state
 *             channel index - 0 / 1 / 2 / 3 /-1(all channel)
 */
...
static BaseType_t radioAutoTXCommand(char *pcWriteBuffer,
        ...
        if(pcParameter1 != NULL && pcParameter2 != NULL){
        /* two parameters: get tx auto mode selection*/

                strncpy ( autotx_param, pcParameter1, xParameter1StringLength);
                mode_sel  = (unsigned int) strtol(pcParameter1, NULL, 10);
                channel   = (unsigned int) strtol(pcParameter2, NULL, 10);
                fmcw_radio_set_auto_tx_mode(NULL, channel, mode_sel);
                readBackLen = sprintf(readBack, "\r\n TX%d Auto mode%d selected\r\n", channel,mode_sel);
                strncat(pcWriteBuffer, readBack, readBackLen);
        } else if(pcParameter1 != NULL && pcParameter2 == NULL) {
        /* one parameters: no tx channel selection*/

                strncpy ( autotx_param, pcParameter1, xParameter1StringLength);
                mode_sel  = (unsigned int) strtol(pcParameter1, NULL, 10);
                fmcw_radio_set_auto_tx_mode(NULL, -1, mode_sel);
                readBackLen = sprintf(readBack, "\r\n all TXs Auto mode%d selected\r\n", mode_sel);
                strncat(pcWriteBuffer, readBack, readBackLen);
        } else {
...
/* radio rf power detector ***********************************************************/

/*
 * command   : radio_pdt
 * parameter : <freq> <ch_index> - return rf power of certain freq and channel
 *
 * note      : channel index - 0 / 1 / 2 / 3 /
 *             freq - in GHz
 */

static const CLI_Command_Definition_t xRadioPDT = {
        "radio_pdt",      /* The command string to type. */
        "\r\nradio_pdt \r\n"
        "\t <ch_index> - 0 / 1 / 2 / 3 /\r\n"
        "\t <mode_sel> - in GHz\r\n",
        radioPDTCommand, /* The function to run.        */
        -1                 /* Up to 2 commands.           */
};

static BaseType_t radioPDTCommand(char *pcWriteBuffer,
        size_t xWriteBufferLen, const char *pcCommandString){

        const char *pcParameter1, *pcParameter2;
        BaseType_t xParameter1StringLength, xParameter2StringLength;
        unsigned int channel;
        double freq, rf_power;
        ...
        if(pcParameter1 != NULL && pcParameter2 != NULL){
        /* two parameters: get channel and frequency */

                channel  = (unsigned int) strtol(pcParameter1, NULL, 10);
                freq   = (double) strtol(pcParameter2, NULL, 10);
                rf_power = fmcw_radio_rf_power_detector(NULL, channel, freq);
                readBackLen = sprintf(readBack, "\r\n TX%d Power at %.2f GHz is %.2f dBm \r\n", channel, freq, rf_power);
                strncat(pcWriteBuffer, readBack, readBackLen);
        } else {
                readBackLen = sprintf(readBack, "\r\n radio pdt parameter error\r\n");
                strncat(pcWriteBuffer, readBack, readBackLen);
        }
...
/* radio tia saturation detector ***********************************************************/
/*
 * command   : radio_tia_sat
 * parameter : <ch_index> - return Vdiff of TIA_N and TIA_P of certain channel
 * note      : channel index - 0 / 1
 */

static const CLI_Command_Definition_t xRadioTiaSat = {
    "radio_tia_sat",      /* The command string to type. */
    "\r\nradio_tia_sat \r\n"
    "\t <ch_diff> - return Vdiff of TIA_N and TIA_P of all channel\r\n"
    "\t <ch_cur_step> - return current gain setup of all channel\r\n"
    "\t <auto_adj ch, threshold_H, threshold_L> - return current gain setup of all channel\r\n",
    radioTiaSatCommand, /* The function to run.        */
    -1                 /* Up to 1 commands.           */
};

static BaseType_t radioTiaSatCommand(char *pcWriteBuffer,
    size_t xWriteBufferLen, const char *pcCommandString){
    ...
    pcParameter3 = FreeRTOS_CLIGetParameter(pcCommandString, 3, &xParameter1StringLength);
    pcParameter4 = FreeRTOS_CLIGetParameter(pcCommandString, 4, &xParameter1StringLength);

    if(pcParameter1 != NULL){
        if (strncmp(pcParameter1,"ch_diff",7) == 0) {
                for (int i = 0; i < MAX_NUM_RX; i++)
                {
                        voltage = fmcw_radio_tia_saturation_detector(radio, i);
                        EMBARC_PRINTF("tia Vdiff at ch%d is: %.3f\n", i, voltage);
                }
        }else if(strncmp(pcParameter1,"ch_cur_step",11) == 0) {
                for (int i = 0; i < MAX_NUM_RX; i++)
                {
                        EMBARC_PRINTF("Current gain step at ch%d is: tia = 0x%02x; pa_ldo = 0x%02x; pa_ldo_bias = 0x%02x\n",
                                      i,
                                      fmcw_radio_get_tia_step(radio, i),
                                      fmcw_radio_get_tx_pa_ldo_step(radio, i),
                                      fmcw_radio_get_tx_pa_bias_step(radio, i));
                }
        }else if(strncmp(pcParameter1,"auto_adj",8) == 0) {
                if ((pcParameter2 != NULL) && (pcParameter3 != NULL) && (pcParameter4 != NULL))
                {
                        channel = strtol(pcParameter2,NULL,0);
                        threshold_H = strtol(pcParameter3,NULL,0);
                        threshold_L = strtol(pcParameter4,NULL,0);
                        EMBARC_PRINTF("auto adj result = %d\n",
                                      fmcw_radio_tia_gain_and_tx_power_auto_detect(radio, channel, ((float)threshold_H)/1000, ((float)threshold_L)/1000));
                }else{
                        EMBARC_PRINTF("\r\n param err,need 3 params:channel,threshold_H(mv),threshold_L(mv) ! \r\n");
                }
                for (int i = 0; i < MAX_NUM_RX; i++)
                {
                        EMBARC_PRINTF("Current gain step at ch%d is: tia = 0x%02x; pa_ldo = 0x%02x; pa_ldo_bias = 0x%02x\n",
        ...
        readBackLen = sprintf(readBack, "\r\n radio_tia_sat parameter error\r\n");
        strncat(pcWriteBuffer, readBack, readBackLen);
        print_help(pcWriteBuffer, xWriteBufferLen, &xRadioTiaSat);
    }
...
void fmcw_radio_cli_commands( void ){
        ...
        #endif
        FreeRTOS_CLIRegisterCommand(&xRadioPDT);
        #if (TIA_SAT == 1)
        FreeRTOS_CLIRegisterCommand(&xRadioTiaSat);
        #endif
        fmcw_radio_cli_registered = true;
}
#endif  // end of "FMCW_RADIO_CLI"