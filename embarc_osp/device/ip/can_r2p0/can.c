...
volatile uint8_t can_node_rec_flag[2] = {0,0};
volatile uint8_t can_tx_done_flag[2] = {1,1};
...
static int32_t can_write_single_frame(can_t *can, uint32_t buf_id, uint32_t *buf, uint32_t frame_size)
{
    ...
    if ((NULL == can) || (buf_id > 63) || (NULL == buf) || (0 == frame_size)) {
       result = E_PAR;
		...
		/*bug1549,clear rx_buf, read node status*/
		can_rx_buffer_clear_all(can);
		if(0x2 == can_protocol_status(can,PROTOCOL_STS_ACTIVE)){
			can_node_rec_flag[can->id] = 1;
		}
