...
static inline void bb_top_enable(uint32_t en)
{
	if (en) {
		raw_writel(REG_CLKGEN_ENA_BB_TOP, 1);
	} else {
		raw_writel(REG_CLKGEN_ENA_BB_TOP, 0);
	}
}

static inline void bb_core_enable(uint32_t en)
{
	if (en) {
		raw_writel(REG_CLKGEN_ENA_BB_CORE, 1);
	} else {
		raw_writel(REG_CLKGEN_ENA_BB_CORE, 0);
	}
}
...
static inline void dbgbus_enable(uint32_t en)
{
	if (en) {
		raw_writel(REG_CLKGEN_ENA_DBGBUS, 1);
	} else {
		raw_writel(REG_CLKGEN_ENA_DBGBUS, 0);
	}
}

static inline void lvds_enable(uint32_t en)
{
	if (en) {
		raw_writel(REG_CLKGEN_ENA_LVDS, 1);
	} else {
		raw_writel(REG_CLKGEN_ENA_LVDS, 0);
	}
}
...
static inline void dma_enable(uint32_t en)
{
	if (en) {
		raw_writel(REG_CLKGEN_ENA_DMA, 1);
	} else {
		raw_writel(REG_CLKGEN_ENA_DMA, 0);
	}
}

static inline void chip_reset(void)
{
	raw_writel(REG_CLKGEN_RSTN_SW_REBOOT, 1);
}
#endif
