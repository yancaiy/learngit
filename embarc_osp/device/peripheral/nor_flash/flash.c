...
int32_t flash_init(void)
{
...
#if FLASH_XIP_EN
	flash_xip_init();
#endif /* Even in XIP mode, the QSPI module also needs to be initialized */
	...
	do {
		...
		result = ssi_ops->enable(dw_ssi, 0);
		if (E_OK != result) {
			break;
		}
		...
		chip_hw_mdelay(10);
		/* close task switch before read flash parameters */
		...
		/* If need quad mode to communicate with flash,updata QSPI to quad mode */
		if (QUAD_FRAME_FORMAT == cur_frame_format) {
			ssi_ops->frf_update(dw_ssi, CONFIG_QSPI_FF);
		}
#ifdef OS_FREERTOS
...
int32_t _flash_memory_erase(uint32_t addr, uint32_t len)
{
	...
	if ((NULL == dw_ssi) || (NULL == dw_ssi->ops) || \
		...
		/* Erase flash need QSPI under STAN_SPI mode */
		if (QUAD_FRAME_FORMAT == cur_frame_format) {
		...
		while (len) {
			...
			if (addr % sector_size) {
				result = E_PAR;
				EMBARC_PRINTF("erase addr invalid ,should aligne by sector size = %x\r\n", sector_size);
				break;
			} else {
			}
		...
		/* After erasing, update QSPI to quad mode if need */
		if (QUAD_FRAME_FORMAT == cur_frame_format) {
...
static int32_t _flash_memory_read(uint32_t addr, void *data, uint32_t len, uint32_t flag)
{
	...
	uint32_t cpu_status = 0;
	uint32_t s_read_cnt = 0;
	dw_ssi_ops_t *ssi_ops = NULL;

	dw_ssi_t *dw_ssi = dw_ssi_get_dev(DW_QSPI_ID);

	dw_ssi_xfer_t xfer = DW_SSI_XFER_INIT(CMD_READ, addr, 0, 1);
	...
	if ((NULL == ext_dev_desc) || \
		...
		if (QUAD_FRAME_FORMAT == cur_frame_format) {
			/* adapt with qspi rx fifo depth */
			s_read_cnt = 0x20;
		}else{
			s_read_cnt = ext_dev_desc->page_size;
		}
		...
		while (len) {
			...
			if (QUAD_FRAME_FORMAT == cur_frame_format) {
				xfer.ins = CMD_Q_READ;
				xfer.rd_wait_cycle = FLASH_DEV_DUMMY_CYCLE + FLASH_DEV_MODE_CYCLE;
			}
			...
			if (flag) {
				addr += (s_read_cnt << 2);
				data = (void *)((uint32_t)data + (s_read_cnt << 2));
			} else {
				addr += s_read_cnt;
...
int32_t flash_get_info(uint32_t addr, uint32_t *sector_size)
{
	int32_t result = E_OK;
	
#if (0 == 1/*SWITCH_CT_TO_VENUS*/)	//??????????
	...
	*sector_size = 4096;	//4k
...
#if 0 // FLASH_XIP_EN
	dw_ssi_ops_t *ssi_ops = NULL;
	dw_ssi_t *dw_ssi = dw_ssi_get_dev(XIP_QSPI_ID);
#else
	dw_ssi_t *dw_ssi = dw_ssi_get_dev(DW_QSPI_ID);
#endif /* !FLASH_XIP_EN! */
	...
	else
	{
		...
		size = ext_dev_desc->m_region[r_idx].sector_size;
		*sector_size = size;
		if (addr % size)
		{
			result = -2;
