...
static can_t *dev_can[CAN_CHANNEL_NUM] = { NULL, NULL};
static can_xfer_t can_buf[CAN_CHANNEL_NUM];
static can_ts can_exa;
static can_ts can_exa_1;
volatile uint8_t first_lost_int_flag[CAN_CHANNEL_NUM] = {1,1};
volatile uint8_t rx_buff_num[CAN_CHANNEL_NUM] = {0,0};
extern volatile uint8_t can_tx_done_flag[CAN_CHANNEL_NUM];
extern volatile uint8_t can_node_rec_flag[CAN_CHANNEL_NUM];
volatile uint32_t assert_reset_flag[CAN_CHANNEL_NUM] = {0,0};
                ...
                if (NULL == dev_can[id]) {
                        ...
                        if (NULL == can_ops) {
                            result = E_NOOPS;
                                ...
                                for (loopcnt = 0; (E_OK == result) && (loopcnt < CAN_ID_FILTER_ELEMENT_NUM); loopcnt++) {
                                        result = can_ops->id_filter_element(dev_can[id], loopcnt, &can_buf[id].filter_element[loopcnt]);
                                }
...
int32_t can_get_config(uint32_t id, void *param)
{
        ...
        for (loopcnt = 0; loopcnt < can_buf[id].filter.filter_size; loopcnt++) {
            if(can_buf[id].filter_element[loopcnt].filter_cfg != eRX_BUF){
                can_conf->std_id_filter.filter_elem[loopcnt].filter_type = can_buf[id].filter_element[loopcnt].filter_type;
                can_conf->std_id_filter.filter_elem[loopcnt].filter_id0  = can_buf[id].filter_element[loopcnt].filter_id0;
                can_conf->std_id_filter.filter_elem[loopcnt].filter_id1  = can_buf[id].filter_element[loopcnt].filter_id1;
            }
        }
        ...
        for (loopcnt = 0; loopcnt < can_buf[id].ext_filter.filter_size; loopcnt++) {
            if(can_buf[id].ext_filter_element[loopcnt].filter_cfg != eRX_BUF){
                can_conf->ext_id_filter.filter_elem[loopcnt].filter_type = can_buf[id].ext_filter_element[loopcnt].filter_type;
                can_conf->ext_id_filter.filter_elem[loopcnt].filter_id0  = can_buf[id].ext_filter_element[loopcnt].filter_id0;
                can_conf->ext_id_filter.filter_elem[loopcnt].filter_id1  = can_buf[id].ext_filter_element[loopcnt].filter_id1;
            }
        }
...
int32_t can_set_config(uint32_t id, void *param)
{
        ...
        for (loopcnt = 0; loopcnt < can_conf->std_id_filter.fiter_config.filter_size; loopcnt++) {
            if (result == E_OK) {
                can_buf[id].filter_element[loopcnt].frame_format = eSTANDARD_FRAME;
                can_buf[id].filter_element[loopcnt].filter_type  = can_conf->std_id_filter.filter_elem[loopcnt].filter_type;
                can_buf[id].filter_element[loopcnt].filter_cfg   = eRX_FIFO;
                can_buf[id].filter_element[loopcnt].filter_id0   = can_conf->std_id_filter.filter_elem[loopcnt].filter_id0;
                can_buf[id].filter_element[loopcnt].filter_id1   = can_conf->std_id_filter.filter_elem[loopcnt].filter_id1;
                result = can_ops->id_filter_element(dev_can[id], loopcnt, &can_buf[id].filter_element[loopcnt]);
                if(result != E_OK){
        ...
        if (result == E_OK) {
            can_buf[id].filter_element[loopcnt].frame_format = eSTANDARD_FRAME;
            can_buf[id].filter_element[loopcnt].filter_type  = eRANGE;
            can_buf[id].filter_element[loopcnt].filter_cfg   = eRX_BUF;
            can_buf[id].filter_element[loopcnt].filter_id0   = 0x0;
            can_buf[id].filter_element[loopcnt].filter_id1   = 0x7ff;
            result = can_ops->id_filter_element(dev_can[id], loopcnt, &can_buf[id].filter_element[loopcnt]);
            if(result != E_OK){
        ...
        for (loopcnt = 0; loopcnt < can_conf->ext_id_filter.fiter_config.filter_size; loopcnt++) {
            if (result == E_OK) {
                can_buf[id].ext_filter_element[loopcnt].frame_format = eEXTENDED_FRAME;
                can_buf[id].ext_filter_element[loopcnt].filter_type  = can_conf->ext_id_filter.filter_elem[loopcnt].filter_type;
                can_buf[id].ext_filter_element[loopcnt].filter_cfg   = eRX_FIFO;
                can_buf[id].ext_filter_element[loopcnt].filter_id0   = can_conf->ext_id_filter.filter_elem[loopcnt].filter_id0;
                can_buf[id].ext_filter_element[loopcnt].filter_id1   = can_conf->ext_id_filter.filter_elem[loopcnt].filter_id1;
                result = can_ops->id_filter_element(dev_can[id], loopcnt, &can_buf[id].ext_filter_element[loopcnt]);
                if(result != E_OK){
        ...
        if (result == E_OK) {
            can_buf[id].ext_filter_element[loopcnt].frame_format = eEXTENDED_FRAME;
            can_buf[id].ext_filter_element[loopcnt].filter_type  = eRANGE;
            can_buf[id].ext_filter_element[loopcnt].filter_cfg   = eRX_BUF;
            can_buf[id].ext_filter_element[loopcnt].filter_id0   = 0x0;
            can_buf[id].ext_filter_element[loopcnt].filter_id1   = 0x1fffffff;
            result = can_ops->id_filter_element(dev_can[id], loopcnt, &can_buf[id].ext_filter_element[loopcnt]);
            if(result != E_OK){
...
static int32_t can_send_frame(can_t *can_device, can_ops_t *dev_ops, can_xfer_t *xfer)
{
        int32_t result = E_OK;
        ...
        can_ops = (can_ops_t *)can_device->ops;
        ...
        do {
                ...
                if (can_buf[can_device->id].mode == eCAN_BUFFER_MODE) {
                ...
                if(xfer->data_tx_mode == DEV_XFER_INTERRUPT){
                        if (can_device->id == 0){
                                if (dw_timer_start(3, 8000000, timeout_handler, (void *)(&can_exa)) != E_OK){
                                    EMBARC_PRINTF("[%s] start timer3 failed \r\n",__func__);
                                }
                        }else{
                                if(dw_timer_start(2, 8000000, timeout_handler, (void *)(&can_exa_1)) != E_OK){
                                    EMBARC_PRINTF("[%s] start timer2 failed \r\n",__func__);
                                }
                        }

                        uint32_t cpu_status = arc_lock_save();
                        result = can_ops->write_frame(can_device, buf_id, frame_buf, buf_element_size);
                        if (E_OK == result) {
                ...
                /* Synchronous mode*/
                        ...
                        for (cnt_1 = 0; cnt_1 < 800; cnt_1++) {
                                if(assert_reset_flag[can_device->id] == 1){
                                    result = E_CLSED;
                                    break;
                                }else if(can_tx_done_flag[can_device->id] == 0){
                                    chip_hw_udelay(100);
                                    result = E_DBUSY;
                                }else{
                                    result = E_OK;
...
static int32_t can_send_frame_isr(can_t *can_device, can_ops_t *dev_ops, can_xfer_t *xfer)
{
        int32_t result = E_OK;
        uint32_t header0 = 0, header1 = 0;
...
#endif
                if (can_device->id == 0){
                        can_exa.dev_can_s = can_device;
                        can_exa.can_buf_s = xfer;
                        if (dw_timer_start(3, 8000000, timeout_handler, (void *)(&can_exa)) != E_OK){
                            EMBARC_PRINTF("[%s] start timer3 failed \r\n",__func__);
                        }
                }else{
                        can_exa_1.dev_can_s = can_device;
        ...
        if ((NULL == can_ptr) || (NULL == can_ops)) {
                ...
                if (eCAN_BUFFER_MODE == mode) {
                        ...
                        if (type == eCAN_INT_TYPE_RX) {
                                int_bitmap = CAN_INT_RX_FIFO_FILLED | CAN_INT_RX_FIFO_LOST;
                                can_ptr->int_line_bitmap[0] = int_bitmap;
                        } else if (type == eCAN_INT_TYPE_ERROR){
                                int_bitmap = CAN_INT_BUS_OFF | CAN_INT_PROTOCOL_ERR;
                                can_ptr->int_line_bitmap[3] = int_bitmap;
...
static void _can_rx_handler(can_t *can_ptr, can_xfer_t *buf, uint32_t line, void *param)
{
        ...
        if ((NULL == can_ptr) || (line >= 4) || \
                ...
                if (NULL == can_ops) {
                        ...
                        int_bitmap = can_ptr->int_line_bitmap[line] | CAN_INT_RAM_ACCESS_FAIL;
        ...
        if (E_OK == result) {
#ifdef CAN_TX_COMPLETE_MONITOR
                if (int_status & (CAN_INT_RX_FIFO_FILLED | CAN_INT_RX_FIFO_LOST)) {
                    if (int_status & CAN_INT_RX_FIFO_LOST) {
                        EMBARC_PRINTF("rx fifo msg lost:%x \r\n",int_status);
                    }
                    /* check receive frame num */
                    val = can_ops->fifo_status(can_ptr, CAN_RX_FIFO, CAN_FIFO_LEVEL);
                    if (val){
                        buf->rx_isr_free_counter += val;
                    }
                }

                /*lost flag worked only when buffer lost int enabled*/
                if (can_ops->rx_lost_int_enabled(can_ptr)){
                    buf->rx_isr_free_counter += (can_ops->rx_buf_frame_num_get(can_ptr) - rx_buff_num[can_ptr->id])* first_lost_int_flag[can_ptr->id];
                    rx_buff_num[can_ptr->id] = can_ops->rx_buf_frame_num_get(can_ptr);
                    if(!(int_status & CAN_INT_RX_FIFO_LOST) && (int_status & CAN_INT_RAM_ACCESS_FAIL)){
                        buf->rx_isr_free_counter += 1;
                        first_lost_int_flag[can_ptr->id] = 0;
                    }
                }
        ...
        if (E_OK == result) {
                    if (int_status & CAN_INT_TX_COMPLISHED) {
                            ...
                            if (buf->rx_isr_free_counter == 32) {
                                    ...
                                    result = E_SYS;
                            } else if (buf->rx_isr_free_counter > 32) {
...
static int32_t can_int_install(uint32_t id)
{
        ...
        if (E_OK == result) {
                if (can_buf[id].mode == eCAN_FIFO_MODE) {
                        ...
                        /* RX FIFO msg lost. select INT0 */
                        result = can_ops->int_line_select(dev_can[id], CAN_INT_RX_FIFO_LOST, 0);
                        if (E_OK != result) {
                                return result;
                        }

                        /* RX Message RAM Access Failure. select INT0 */
                        result = can_ops->int_line_select(dev_can[id], CAN_INT_RAM_ACCESS_FAIL, 0);
                        if (E_OK != result) {
                                return result;
                        }
