...
{
	int32_t existed = 0;
	uint32_t i, sdu_cnt = cantp.nof_nsdu;
...
static int32_t cantp_send_frame(cantp_channel_t *tx_chn, cantp_nsdu_t *tx_nsdu)
{
	...
	uint32_t buffidx, len = tx_nsdu->frame_length - tx_chn->buffer.index;
	...
	if (tx_chn->msg_data) {
		...
		if (E_OK == result) {
			...
			buffidx = tx_chn->buffer.index;
			if (tx_nsdu->padding_active) {
				for (; buffidx < tx_nsdu->frame_length; buffidx++) {
					tx_chn->buffer.data[buffidx] = 0x0;
				}
			}
...
{
	uint32_t i, existed = 0;

	cantp_channel_t *tx_chn = NULL;

	for (i = 0; i < CAN_BUS_CONTROLLER_CNT; i++) {
		tx_chn = &cantp.connect[i].tx_chn;
...
static int32_t cantp_sf_handle(cantp_channel_t *rx_chn, cantp_nsdu_t *rx_nsdu, pdu_t *pdu)
{
	...
	if (E_OK == result) {
		...
		if (E_DBUSY == result) {
			uint32_t loopcnt = 0;
			for (; loopcnt < data_len; loopcnt++) {
				rx_chn->buffer.data[loopcnt] = pdu->data[data_pos++];
			}
...
static int32_t cantp_ff_handle(cantp_channel_t *rx_chn, cantp_nsdu_t *rx_nsdu, pdu_t *pdu)
{
	...
	if (E_OK == result) {
		...
		if (E_DBUSY == result) {
			uint32_t loopcnt = 0;
			data_pos += 1;
			for (; loopcnt < pdu->length - data_pos; loopcnt++) {
				rx_chn->buffer.data[loopcnt] = pdu->data[data_pos++];
			}
			rx_chn->buffer.length = data_len;
...
static int32_t cantp_fc_handle(cantp_channel_t *tx_chn, cantp_nsdu_t *tx_nsdu, pdu_t *pdu)
{
	...
	if ((NULL == tx_chn) || (NULL == tx_nsdu) || (NULL == pdu)) {
		...
		/* extend addressing. */
		if (tx_nsdu->addr_xtd) {
			uint32_t xtd_addr = pdu->data[loopcnt++];
	...
	if ((E_OK == result) && (CANTP_TX_WAIT_FC == tx_chn->state)) {
		...
		uint32_t fc_status = pdu->data[loopcnt++] & 0xF;

		switch (fc_status) {
			case CANTP_FS_CTS:
				tx_chn->xfer_ctrl.bs = pdu->data[loopcnt++];
				tx_chn->xfer_ctrl.next_fc_cnt = tx_chn->xfer_ctrl.bs;
				tx_chn->xfer_ctrl.st_min = pdu->data[loopcnt++];
...
static void cantp_send_fc(cantp_channel_t *rx_chn, cantp_nsdu_t *rx_nsdu, uint32_t status)
{
	...
	if (rx_nsdu->addr_xtd) {
		//fc_data[loopcnt++] = rx_nsdu->n_sa;
		fc_data[loopcnt++] = rx_nsdu->s_addr;
	}
	...
	switch (status) {
		case CANTP_FS_CTS:
			...
			/* send bs and st_min. */
			fc_data[loopcnt++] = CANTP_PCI_FC | CANTP_FS_CTS;
			if (rx_nsdu->addr_xtd) {
				fc_data[loopcnt++] = rx_chn->xfer_ctrl.upper_buf_size / 6;
			} else {
				fc_data[loopcnt++] = rx_chn->xfer_ctrl.upper_buf_size / 7;
			}
			rx_chn->xfer_ctrl.bs = fc_data[loopcnt - 1];
			rx_chn->xfer_ctrl.next_fc_cnt = rx_chn->xfer_ctrl.bs;
			fc_data[loopcnt++] = rx_nsdu->st_min;
			pdu.length = loopcnt;
			rx_chn->xfer_ctrl.wft_max_cnt = 0;
			break;

		case CANTP_FS_WAIT:
			fc_data[loopcnt++] = CANTP_PCI_FC | CANTP_FS_WAIT;
			pdu.length = loopcnt + 2;
			rx_chn->xfer_ctrl.wft_max_cnt++;
			break;

		case CANTP_FS_OVERFLOW:
			fc_data[loopcnt++] = CANTP_PCI_FC | CANTP_FS_OVERFLOW;
			pdu.length = loopcnt + 2;
			break;
	...
	if (E_OK == result) {
		if (rx_nsdu->padding_active) {
			for (loopcnt = pdu.length; loopcnt < rx_nsdu->frame_length; loopcnt++) {
				pdu.data[loopcnt] = 0x55;
			}
