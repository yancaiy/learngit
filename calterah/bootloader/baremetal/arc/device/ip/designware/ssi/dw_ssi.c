...
static void dw_ssi_data_frame_config(dw_ssi_t *dw_ssi, uint32_t dfs, \
		uint32_t ins_len, uint32_t addr_len, uint32_t wait_cycle, uint32_t transfer_type)
{
	...
	value &= ~(BIT_DW_SSI_SPI_CTRLR0_TRANS_TYPE_MASK << BIT_DW_SSI_SPI_CTRLR0_TRANS_TYPE_SHIFT);
	...
	value |= (transfer_type & BIT_DW_SSI_SPI_CTRLR0_TRANS_TYPE_MASK) << BIT_DW_SSI_SPI_CTRLR0_TRANS_TYPE_SHIFT;
...
static int32_t dw_ssi_xfer(dw_ssi_t *dw_ssi, uint32_t *rx_buf, uint32_t *tx_buf, uint32_t len)
{
	...
	if ((NULL == dw_ssi) || (0 == len) || ((NULL == tx_buf) && (NULL == rx_buf))) {
		...
		if (DEV_MASTER_MODE == dw_ssi->mode) {
			...
			result = dw_ssi_interrupt_status(dw_ssi, SSI_INT_STS_RX_FIFO_FULL);

			/* Clean up the RX FIFO */
			if (result)
			{
				/* If the RX INT is active ,reserve the valid data in RX FIFO */
				while (raw_readl(reg_addr) & BIT_DW_SSI_SR_RFNE) {
					*rx_buf++ = raw_readl(dw_ssi->base + REG_DW_SSI_DR_OFFSET(0));
					len--;
				}
			} else {
				while (raw_readl(reg_addr) & BIT_DW_SSI_SR_RFNE) {
...
static void dw_ssi_ro_read(dw_ssi_t *dw_ssi, dw_ssi_xfer_t *xfer, uint32_t flag)
{
	if ((NULL != dw_ssi) && (NULL != xfer)) {
		...
		while (length) {
			...
			if (xfer->addr_valid) {
				/* For quad mode read ,if address width is 32bits, change to (xfer->addr) & 0xFFFFFFFF
				note: do not split the address into several times */
				dw_ssi_putdata(dw_ssi, (xfer->addr) & 0xFFFFFF);
			}

			loop_cnt = single_read_cnt;
			/* select salve: start to transfer data. */
			raw_writel(REG_DW_SSI_SER_OFFSET + dw_ssi->base, 1);
			/* flag meaning: 1 -> word read; 0 -> byte read */
			if (flag) {
				while (loop_cnt--) {
...
static int32_t dw_ssi_read(dw_ssi_t *dw_ssi, dw_ssi_xfer_t *xfer, uint32_t flag)
{
	...
	if ((NULL == dw_ssi) || (NULL == xfer) || (NULL == dw_ssi->ops) || \
		...
		if (frf_mode == QUAD_FRAME_FORMAT) {
			ctrl_desc.transfer_type = DW_SSI_1_X_X;
		}
...
static void dw_ssi_to_write(dw_ssi_t *dw_ssi, dw_ssi_xfer_t *xfer, uint32_t flag)
{
	if ((NULL != dw_ssi) || (NULL != xfer)) {
		...
		while (raw_readl(REG_DW_SSI_SR_OFFSET + dw_ssi->base) & BIT_DW_SSI_SR_TFNF) {
			if (length) {
				/* flag meaning: 1 -> word write; 0 -> byte write */
				if (flag) {
					raw_writel(dw_ssi->base + REG_DW_SSI_DR_OFFSET(0), *wdata_ptr++);
				}
				else {
		...
		/* write data! flag meaning: 1 -> word write; 0 -> byte write */
		if (flag) {
			while (length--) {
				dw_ssi_putdata(dw_ssi, *wdata_ptr++);
			}
...
/* switch APB QSPI frame format */
static void dw_ssi_frf_update(dw_ssi_t *dw_ssi, uint32_t qspi_frf)
{
	raw_writel(REG_DW_SSI_SSIENR_OFFSET + dw_ssi->base, 0);
	uint32_t value = raw_readl(REG_DW_SSI_CTRLR0_OFFSET + dw_ssi->base);
	value &= ~((0x3 & BITS_DW_SSI_CTRLR0_SPI_FRF_MASK) << BITS_DW_SSI_CTRLR0_SPI_FRF_SHIFT);
	value |= (qspi_frf & BITS_DW_SSI_CTRLR0_SPI_FRF_MASK) << BITS_DW_SSI_CTRLR0_SPI_FRF_SHIFT;
	raw_writel(REG_DW_SSI_CTRLR0_OFFSET + dw_ssi->base, value);
	raw_writel(REG_DW_SSI_SSIENR_OFFSET + dw_ssi->base, 1);
}
/* get APB QSPI frame format */
static uint32_t dw_ssi_frf_get(dw_ssi_t *dw_ssi) {

	uint32_t frf_config = 0;

	uint32_t value = raw_readl(REG_DW_SSI_CTRLR0_OFFSET + dw_ssi->base);

	frf_config = (value >> BITS_DW_SSI_CTRLR0_SPI_FRF_SHIFT) & BITS_DW_SSI_CTRLR0_SPI_FRF_MASK;
...
static dw_ssi_ops_t dw_ssi_ops = {
	...
	.frf_update = dw_ssi_frf_update,
	.frf_get = dw_ssi_frf_get,
