...
void uds_dispatch_init(void)
{
	...
	do {
		...
		for (; index < can_uds.nof_diagnostic; index++) {
			protocol = can_uds.diagnostic_list[index].uds_protocol;
			if (NULL != protocol) {
				/* not started. */
...
void uds_sd_state_main(void)
{
	...
	for (; index < can_uds.nof_diagnostic; index++) {
		cur_diag = &can_uds.diagnostic_list[index];
		if (NULL == cur_diag) {
			continue;
		}
