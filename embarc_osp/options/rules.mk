...
bin : $(APPL_FULL_NAME).bin $(APPL_FULL_NAME).ini $(APPL_FULL_NAME).dump
...
.SECONDEXPANSION:
$(APPL_FULL_NAME).elf : $(ALL_GENERATED_DIRS_TMPFILES) $(CORE_ARG_FILES) $(APPL_LINK_FILE) $(APPL_OBJS) $(EXTRA_OBJS) $(EMBARC_LIB) $(CALTERAH_COMMON_LIBS) $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_LINK)
	$(Q)$(LD) $(LINK_OPT) $(APPL_OBJS) $(EXTRA_OBJS) $(LD_START_GROUPLIB) $(EMBARC_LIB) $(APPL_LIBS) $(MATH_LIB) $(LD_SYSTEMLIBS) $(CALTERAH_COMMON_LIBS) $(LD_END_GROUPLIB) -o $@
...
# add compiling flag workaround for ALPS-A
.SECONDEXPANSION:
$(OUT_DIR)/chip/%.o :$(EMBARC_ROOT)/chip/%.c $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_COMPILE)
	$(Q)$(CC) -c $(COMPILE_OPT) $(COMPILE_HW_OPT) $< -o $@

# add compiling flag workaround for ALPS-A
.SECONDEXPANSION:
$(OUT_DIR)/device/%.o :$(EMBARC_ROOT)/device/%.c $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_COMPILE)
	$(Q)$(CC) -c $(COMPILE_OPT) $(COMPILE_HW_OPT) $< -o $@
