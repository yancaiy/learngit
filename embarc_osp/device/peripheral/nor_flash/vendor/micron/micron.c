...
static int32_t micron_param_read(dw_ssi_t *dw_ssi);
static int32_t micron_quad_entry(dw_ssi_t *dw_ssi);
static int32_t micron_quad_exit(dw_ssi_t *dw_ssi);
static int32_t micron_qpi_entry(dw_ssi_t *dw_ssi);
static int32_t micron_qpi_exit(dw_ssi_t *dw_ssi);
static int32_t micron_status_read(dw_ssi_t *dw_ssi, uint32_t status);

static flash_dev_ops_t micron_ops = {
	.quad_entry = micron_quad_entry,
	.quad_exit = micron_quad_exit,
	.qpi_entry = micron_qpi_entry,
	.qpi_exit = micron_qpi_exit,

	.status = micron_status_read
};

#ifdef STATIC_EXT_FLASH_PARAM
static flash_device_t micron_dev = {
...
flash_device_t *get_flash_device(dw_ssi_t *dw_ssi)
{
	...
	if (0 == ext_dev_open_flag) {
		result = micron_param_read(dw_ssi);
		if (E_OK == result) {
			ext_dev = &micron_dev;
			ext_dev->ops = &micron_ops;
		}
	} else {
		ext_dev = &micron_dev;
	}
...
#if 0
int32_t micron_status_read_test(dw_ssi_t *dw_ssi)
{
	int32_t result = E_OK;
