...
/* GD25Q32C and GD25Q127C */

static int32_t issi_param_read(dw_ssi_t *dw_ssi);
static int32_t issi_quad_entry(dw_ssi_t *dw_ssi);
static int32_t issi_quad_exit(dw_ssi_t *dw_ssi);
static int32_t issi_qpi_entry(dw_ssi_t *dw_ssi);
static int32_t issi_qpi_exit(dw_ssi_t *dw_ssi);
static int32_t issi_status_read(dw_ssi_t *dw_ssi, uint32_t status);

static flash_dev_ops_t issi_ops = {
	.quad_entry = issi_quad_entry,
	.quad_exit = issi_quad_exit,
	.qpi_entry = issi_qpi_entry,
	.qpi_exit = issi_qpi_exit,

	.status = issi_status_read
};

#ifdef STATIC_EXT_FLASH_PARAM
static flash_device_t issi_dev = {
...
flash_device_t *get_flash_device(dw_ssi_t *dw_ssi)
{
	...
	if (0 == ext_dev_open_flag) {
		result = issi_param_read(dw_ssi);
		if (E_OK == result) {
			ext_dev = &issi_dev;
			ext_dev->ops = &issi_ops;
		}
	} else {
		ext_dev = &issi_dev;
	}
    ...
    do {
        if ((NULL == dw_ssi) || (NULL == dw_ssi->ops)) {
            ...
            while ((E_OK == result) && (cnt--)) {
                chip_hw_mdelay(1);
                result = issi_status_read(dw_ssi, FLASH_WEL);
            }
            if (!result) {
                result = E_TMOUT;
                break;
            ...
            do {
                chip_hw_mdelay(1);
                result = issi_status_read(dw_ssi, FLASH_WIP);
            } while((E_OK != result) && (--cnt));
        }
    } while (0);
...
#if 0
int32_t issi_status_read_test(dw_ssi_t *dw_ssi)
{
	int32_t result = E_OK;
