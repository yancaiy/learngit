...
dw_timer_t *dw_timer = NULL;
static timer_callback expire_callback[CHIP_TIMER_NO];
static timer_mode_t dw_timer_mode[CHIP_TIMER_NO] = {USER_DEFINED_MODE,USER_DEFINED_MODE,USER_DEFINED_MODE,USER_DEFINED_MODE};
...
{
    dw_timer_ops_t *timer_ops = NULL;

    if(id >= CHIP_TIMER_NO)
    {
        return E_NOID;
    }
    if ((mode != USER_DEFINED_MODE) && (mode != FREE_RUNNING_MODE))
    {
        return E_PAR;
    }

    dw_timer_mode[id] = mode;

    if(dw_timer != NULL)
    {
        timer_ops = (dw_timer_ops_t *)dw_timer->ops;
        if(timer_ops != NULL)
        {
            return timer_ops->mode(dw_timer,id,dw_timer_mode[id]);
        }
    }
    return E_OK;
}
int32_t dw_timer_start(uint32_t id, uint32_t expire, void (*func)(void *param),void *param)
{
    ...
    if ((NULL == dw_timer) || (NULL == dw_timer->ops) || \
        ...
        do {
            if (expire) {
                /* user define mode. */
                result = timer_ops->mode(dw_timer, id, dw_timer_mode[id]);
                if (E_OK != result) {
                    break;
                }
            ...
            dw_timer->usr_param[id] = (void *)param;
		...
		for (timer_id = 0; timer_id < dw_timer->timer_cnt; timer_id++) {
			...
			if (result > 0) {
				if (expire_callback[timer_id]) {
					expire_callback[timer_id]((void *)dw_timer->usr_param[timer_id]);
				}
