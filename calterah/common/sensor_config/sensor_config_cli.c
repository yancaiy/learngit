...
BaseType_t sensor_cfg_command_handler(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString);
...
static BaseType_t sensor_cfg_sel_handler(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
        ...
        if (chip_cascade_status() == CHIP_CASCADE_MASTER) {
                EMBARC_PRINTF(pcCommandString);
                EMBARC_PRINTF(" sent to slave \n");
                baseband_write_cascade_cmd(pcCommandString); // send "sensor_cfg_sel" command string to slave chip through spi in cascade mode
        }
#endif
...
#endif // end of "SENSOR_CONFIG_CLI"
