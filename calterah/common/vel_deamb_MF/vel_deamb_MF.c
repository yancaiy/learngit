...
static signed char wrp_num_tbl[TRACK_NUM_CDI]; // A table storing Doppler domain wrap number

/*
 * Get wrap number of a certain target
 * */
signed char read_wrap_num(unsigned char obj_ind)
{
        return wrp_num_tbl[obj_ind];
}

/*
 * Write wrap number of detected targets into MEM_RLT for velocity compensation when running MIMO DoA
 * */
void write_wrap_num(baseband_t *bb)
{
        baseband_hw_t *bb_hw = &bb->bb_hw;
        ...
        uint32_t old = baseband_switch_mem_access(bb_hw, SYS_MEM_ACT_RLT);
        uint32_t mem_rlt_offset = 1 * (1<<SYS_SIZE_OBJ_WD_BNK) * RESULT_SIZE;
        volatile obj_info_t *obj_info = (volatile obj_info_t *)(BB_MEM_BASEADDR + current_frame_type * mem_rlt_offset);// Starting address of objects infos

        for(int j = 0; j < obj_num ; j++) {

                wrp_num_tbl[j] = 0;

                uint32_t noi_val = obj_info[j].noi;
                uint32_t * p_tmp = (uint32_t *)(&(obj_info[j]));
                unsigned char amb_idx_offset = 1;// memory storage offset of amb_idx in obj_info structure, unit : word

                if (1) {  // Write wrap_num into segment 'amb_idx' in MEM_RLT
                        int wrp_num = 0; // calculated wrap number
                        // write wrp_num to obj_info[j].amb_idx, reorganize as noi(24:5), amb_idx(4:0) due to R/W difference
                        *(p_tmp + amb_idx_offset) = (noi_val<<5) + (unsigned char)((wrp_num)&0x1F);
                        wrp_num_tbl[j] = wrp_num;
                }
