...
volatile uint8_t can_node_rec_flag[CAN_CHANNEL_NUM] = {0,0};
volatile uint8_t can_tx_done_flag[CAN_CHANNEL_NUM] = {1,1};
extern volatile uint8_t rx_buff_num[CAN_CHANNEL_NUM];
extern volatile uint8_t first_lost_int_flag[CAN_CHANNEL_NUM];
...
static int32_t can_baud_rate(can_t *can, uint32_t fd_enabled, can_baud_t *param)
{
    ...
    if ( (NULL == can) || (NULL == param) ){
        ...
        /* Unsupport Baudrate Configure:
         * the minimum time quantum config is not supported.Refer to Bug1270.
         */
        if ( (param->bit_rate_prescale + 1) * (param->segment1 + param->segment2 + 3) < 20) {
            return E_PAR;
        }
        ...
        if (fd_enabled) {
            ...
            val |= (param->segment1 & BITS_DATA_BIT_TIME_CTRL_DTSG1_MASK) << \
                    BITS_DATA_BIT_TIME_CTRL_DTSG1_SHIFT;
            val |= (param->segment2 & BITS_DATA_BIT_TIME_CTRL_DTSG2_MASK) << \
                    BITS_DATA_BIT_TIME_CTRL_DTSG2_SHIFT;
        } else {
            /* Unsupport Baudrate Configure:
             * When nbrp is set to 2, CAN tx point is not generated right,
             * which will cause CAN tx output data error.Refer to Bug1269.
             */
            if ( (param->bit_rate_prescale == 2) && (param->segment1 == 7) && (param->segment2 == 1) ) {
                return E_PAR;
            }
            reg += REG_CAN_NML_BIT_TIME_CTRL_OFFSET;
            val |= (param->sync_jump_width & BITS_NML_BIT_TIME_CTRL_NSJW_MASK) << \
                    BITS_NML_BIT_TIME_CTRL_NSJW_SHIFT;
            val |= (param->bit_rate_prescale & BITS_NML_BIT_TIME_CTRL_NBRP_MASK) << \
                    BITS_NML_BIT_TIME_CTRL_NBRP_SHIFT;
            val |= (param->segment1 & BITS_NML_BIT_TIME_CTRL_NTSEG1_MASK) << \
                    BITS_NML_BIT_TIME_CTRL_NTSEG1_SHIFT;
            val |= (param->segment2 & BITS_NML_BIT_TIME_CTRL_NTSEG2_MASK) << \
                    BITS_NML_BIT_TIME_CTRL_NTSEG2_SHIFT;
        }
...
static int32_t can_timeout_config(can_t *can, can_timeout_op_t enable, uint32_t period, can_timeout_sel_t sel)
{
	int32_t result = E_OK;
	uint32_t val = 0;
	...
	if ( (NULL == can) ) {
		...
		val |= (sel & BITS_TIMEOUT_COUNTER_TOS_MASK) << BITS_TIMEOUT_COUNTER_TOS_SHIFT;

		if (enable == eCAN_TIMEOUT_ENABLE) {
...
static int32_t can_timeout_counter_get(can_t *can, uint32_t *counter_value)
{
	int32_t result = E_OK;
	...
	if ( (NULL == can) ) {
		...
		uint32_t reg = REG_CAN_TIMEOUT_COUNTER_VAL_OFFSET + can->base;

		*counter_value = raw_readl(reg);
	}
...
static int32_t can_interrupt_status(can_t *can, uint32_t mask)
{
	...
	if ( (NULL == can) ) {
		...
		uint32_t reg = REG_CAN_ISR_OFFSET + can->base;
		val = raw_readl(reg);
		result = val & mask;
	}
...
static int32_t can_interrupt_enable(can_t *can, uint32_t enable, uint32_t mask)
{
	...
	if ( (NULL == can) ) {
		...
		uint32_t reg = REG_CAN_ISR_ENABLE_OFFSET + can->base;
		val = raw_readl(reg);
		if (enable) {
			val |= mask;
		}  else {
...
static int32_t can_rx_lost_int_enabled(can_t *can)
{
    int32_t result = E_OK;
    uint32_t val = 0;
    ...
    if ( (NULL == can) ) {
        ...
        uint32_t reg = REG_CAN_ISR_OFFSET + can->base;
        val = raw_readl(reg);
        result = val & CAN_INT_RAM_ACCESS_FAIL;
    }
...
static int32_t can_interrupt_line_select(can_t *can, uint32_t int_source, uint32_t int_line)
{
	...
	if ( (NULL == can) ) {
		...
		uint32_t sel0 = REG_CAN_ISR_LINE_SELECT0_OFFSET + can->base;
		uint32_t sel1 = REG_CAN_ISR_LINE_SELECT1_OFFSET + can->base;
		switch (int_source) {
			case CAN_INT_TIMEOUT_OCCURED:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_TOOLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TOOLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_ACCESS_PROTECT_REG:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_APRLS_MASK) << \
				       BITS_ISR_LINE_SELECT_APRLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_PROTOCOL_ERR:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_PEDLS_MASK) << \
				       BITS_ISR_LINE_SELECT_PEDLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_BUS_OFF:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_BOLS_MASK) << \
				       BITS_ISR_LINE_SELECT_BOLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_WARNING:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_EWLS_MASK) << \
				       BITS_ISR_LINE_SELECT_EWLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_ERR_PASSIVE:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_EPLS_MASK) << \
				       BITS_ISR_LINE_SELECT_EPLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_ERR_LOGGING_OVERFLOW:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_ELOLS_MASK) << \
				       BITS_ISR_LINE_SELECT_ELOLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_BIT_ERR_UNCORRECTED:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_BEULS_MASK) << \
				       BITS_ISR_LINE_SELECT_BEULS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_BIT_ERR_CORRECTED:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_BECLS_MASK) << \
				       BITS_ISR_LINE_SELECT_BECLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_RAM_ACCESS_FAIL:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_MRAFLS_MASK) << \
				       BITS_ISR_LINE_SELECT_MRAFLS_SHIFT;
				raw_writel(sel0, val);
				break;
			case CAN_INT_TIMESTAMP_WRAP_AROUND:
				val = raw_readl(sel0);
				val |= (int_line & BITS_ISR_LINE_SELECT_TSWLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TSWLS_SHIFT;
				raw_writel(sel0, val);
				break;

			case CAN_INT_RX_NEW_MESSAGE:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_RXBNLS_MASK) << \
				       BITS_ISR_LINE_SELECT_RXBNLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_RX_FIFO_LOST:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_RXFLLS_MASK) << \
				       BITS_ISR_LINE_SELECT_RXFLLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_RX_FIFO_REAL_FULL:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_RXFFLS_MASK) << \
				       BITS_ISR_LINE_SELECT_RXFFLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_RX_FIFO_FULL:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_RXFWLS_MASK) << \
				       BITS_ISR_LINE_SELECT_RXFWLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_RX_FIFO_EMPTY:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_RXFELS_MASK) << \
				       BITS_ISR_LINE_SELECT_RXFELS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_CANCEL_FINISHED:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TXBCLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TXBCLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_COMPLISHED:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TXBTLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TXBTLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_FIFO_LOST:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TXFLLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TXFLLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_FIFO_REAL_FULL:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TXFFLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TXFFLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_FIFO_FULL:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TXFWLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TXFWLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_FIFO_EMPTY:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TXFELS_MASK) << \
				       BITS_ISR_LINE_SELECT_TXFELS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_EVENT_FIFO_LOST:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TEFLLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TEFLLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_EVENT_FIFO_REAL_FULL:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TEFFLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TEFFLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_EVENT_FIFO_FULL:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TEFWLS_MASK) << \
				       BITS_ISR_LINE_SELECT_TEFWLS_SHIFT;
				raw_writel(sel1, val);
				break;
			case CAN_INT_TX_EVENT_FIFO_EMPTY:
				val = raw_readl(sel1);
				val |= (int_line & BITS_ISR_LINE_SELECT_TEFELS_MASK) << \
				       BITS_ISR_LINE_SELECT_TEFELS_SHIFT;
				raw_writel(sel1, val);
				break;
			default:
				result = E_PAR;
...
static int32_t can_interrupt_line_enable(can_t *can, uint32_t int_line, uint32_t enable)
{
	...
	if ( (NULL == can) ) {
		...
		uint32_t reg = REG_CAN_ISR_LINE_ENABLE_OFFSET + can->base;
		val = raw_readl(reg);
		if (enable) {
			val |= 1 << (int_line & 0x3);
		} else {
...
static int32_t can_id_filter_standard_element(can_t *can, uint32_t index, can_id_filter_element_t *ele_cfg)
{
	...
	if ( (NULL == can) ) {
		...
		sub_element |= ((ele_cfg->filter_type & BITS_STD_ID_FILTER_EL_SFT_MSASK) << \
						BITS_STD_ID_FILTER_EL_SFT_SHIFT);
		sub_element |= ((ele_cfg->filter_cfg & BITS_STD_ID_FILTER_EL_SFEC_MASK) << \
						BITS_STD_ID_FILTER_EL_SFEC_SHIT);
		sub_element |= ((ele_cfg->filter_id0 & BITS_STD_ID_FILTER_EL_SFID0_MASK) << \
						BITS_STD_ID_FILTER_EL_SFID0_SHIFT);
		sub_element |= ((ele_cfg->filter_id1 & BITS_STD_ID_FILTER_EL_SFID1_MASK) << \
						BITS_STD_ID_FILTER_EL_SFID1_SHIFT);
...
static int32_t can_id_filter_extended_element(can_t *can, uint32_t index, can_id_filter_element_t *ele_cfg)
{
	...
	if ( (NULL == can) ) {
		...
		element_f0 |= ((ele_cfg->filter_type & BITS_EXT_ID_FILTER_EL_EFT_MASK) << \
						BITS_EXT_ID_FILTER_EL_EFT_SHIFT);
		element_f0 |= ((ele_cfg->filter_id0 & BITS_EXT_ID_FILTER_EL_EFID0_MASK) << \
						BITS_EXT_ID_FILTER_EL_EFID0_SHIFT);
		element_f1 |= ((ele_cfg->filter_cfg & BITS_EXT_ID_FILTER_EL_EFEC_MASK) << \
						BITS_EXT_ID_FILTER_EL_EFEC_SHIFT);
		element_f1 |= ((ele_cfg->filter_id1 & BITS_EXT_ID_FILTER_EL_EFID1_MASK) << \
						BITS_EXT_ID_FILTER_EL_EFID1_SHIFT);

		raw_writel(reg, element_f0);
		reg += REG_CAN_EXT_ID_FILTER_EL_F1_OFFSET;
...
static int32_t can_id_filter_element_config(can_t *can, uint32_t index, can_id_filter_element_t *ele_cfg)
{
    int32_t result = E_OK;
    result = can_opmode_config(can, CONFIG_MODE);
    /*Note:bug1274,filter_cfg can't be eNUM_OF_CAN_STD_FILTER_ELEMENT_CFG(means reserved)*/
    if ( (NULL == can) || (NULL == ele_cfg) || (E_OK != result) || (ele_cfg->filter_cfg >= eNUM_OF_CAN_STD_FILTER_ELEMENT_CFG)) {
        result = E_PAR;
    } else {
...
static int32_t can_id_filter_standard_frame(can_t *can, can_id_filter_param_t *param)
{
    ...
    if ( (NULL == can) ) {
        ...
        val &= ~BITS_ID_FILTER_CTRL_SIDFS_MASK;
...
static int32_t can_id_filter_extended_frame(can_t *can, can_id_filter_param_t *param)
{
    ...
    if ( (NULL == can) ) {
        ...
        val &= ~(BITS_ID_FILTER_CTRL_XIDFS_MASK << BITS_ID_FILTER_CTRL_XIDFS_SHIFT);
...
static int32_t can_id_filter_config(can_t *can, can_id_filter_param_t *param)
{
    ...
    result = can_opmode_config(can, CONFIG_MODE);
    /* Note:bug1300,up to MAX_CAN_FILTER_NUM filters can be enabled */
    if ( (NULL == can) || (NULL == param) || (E_OK != result) || (param->filter_size > MAX_CAN_FILTER_NUM)) {
        result = E_PAR;
    } else {
...
static int32_t can_rx_data_field_size(can_t *can, uint32_t fifo_dfs, uint32_t buf_dfs)
{
	...
	if ( (NULL == can) ) {
		...
		uint32_t reg = REG_CAN_RX_EL_SIZE_CFG_OFFSET + can->base;
		val |= (fifo_dfs & BITS_RX_EL_SIZE_CFG_RXFDS_MASK) << \
				BITS_RX_EL_SIZE_CFG_RXFDS_SHIFT;
		val |= ( buf_dfs & BITS_RX_EL_SIZE_CFG_RXBDS_MASK) << \
				BITS_RX_EL_SIZE_CFG_RXBDS_SHIFT;
		raw_writel(reg, val);
	}
...
static int32_t can_tx_data_field_size(can_t *can, uint32_t fifo_dfs, uint32_t buf_dfs)
{
	...
	if ( (NULL == can) ) {
		...
		uint32_t reg = REG_CAN_TX_EL_SIZE_CFG_OFFSET + can->base;
		val |= (fifo_dfs & BITS_TX_EL_SIZE_CFG_TXFDS_MASK) << \
				BITS_TX_EL_SIZE_CFG_TXFDS_SHIFT;
		val |= (buf_dfs & BITS_TX_EL_SIZE_CFG_TXBDS_MASK) << \
				BITS_TX_EL_SIZE_CFG_TXBDS_SHIFT;
		raw_writel(reg, val);
	}
...
static int32_t can_tx_buffer_add_request_status(can_t *can, uint32_t buf_id)
{
    int32_t result = E_OK;
    uint32_t val = 0;
...
static int32_t can_rx_buffer_clear_all(can_t *can)
{
	int32_t result = E_OK;
	...
	if (NULL == can) {
		...
		raw_writel(REG_CAN_RX_BUFFER_STATUS0_OFFSET + can->base,0xffffffff);
		raw_writel(REG_CAN_RX_BUFFER_STATUS1_OFFSET + can->base,0xffffffff);
	}

	return result;
}

static int32_t can_rx_buf_size_get(can_t *can)
{
    int32_t result = E_OK;
    uint32_t val = 0;
    ...
    if(result == E_OK){
        val = (raw_readl(REG_CAN_RX_EL_SIZE_CFG_OFFSET + can->base) & BITS_RX_EL_SIZE_CFG_RXBDS_MASK) >> BITS_RX_EL_SIZE_CFG_RXBDS_SHIFT;
...
static int32_t can_rx_buf_frame_num_get(can_t *can)
{
    int32_t result = E_OK;
    uint32_t val = 0;
    ...
    if (NULL == can) {
        ...
        for (int i = 0;i < 32;i++){
            result += (raw_readl(REG_CAN_RX_BUFFER_STATUS0_OFFSET + can->base) >> i) & 0x1;
        }
        for (int j = 0;j < 32;j++){
            result += (raw_readl(REG_CAN_RX_BUFFER_STATUS1_OFFSET + can->base) >> j) & 0x1;
        }

        val = can_rx_buf_size_get(can);
        if(result > val){
            result = val;
        }
    }
...
static int32_t can_write_single_frame(can_t *can, uint32_t buf_id, uint32_t *buf, uint32_t frame_size)
{
    ...
    if ((NULL == can) || (buf_id > 63) || (NULL == buf) || (0 == frame_size)) {
       result = E_PAR;
        ...
        /*clear can_tx_done_flag*/
        can_tx_done_flag[can->id] = 0;
        if(first_lost_int_flag[can->id]){
            rx_buff_num[can->id] = can_rx_buf_frame_num_get(can);
        }

        /* enable rx lost int */
        can_interrupt_enable(can, 1, CAN_INT_RAM_ACCESS_FAIL);
...
/* Note:there must be 100 clock cycles between two fifo operations.refer to bug1283 & bug1295 */
static int32_t can_fifo_read(can_t *can, uint32_t *buf, uint32_t len)
{
    int32_t result = E_OK;
    int32_t value = 0;
    uint32_t reg = 0;
    ...
    if ( (NULL == can) ) {
        ...
        reg = REG_CAN_RX_FIFO_EL_OFFSET(0) + can->base;
    }

    value = can_fifo_status(can, CAN_RX_FIFO, CAN_FIFO_EMPTY);
    if (value){
        result = E_NODATA;
        return result;
    }
    ...
    if (E_OK == result) {
        while (len--) {
            *buf++ = raw_readl(reg);
            ...
            /* The parameter len is equal to FIFO Element Size. */
            /* Its unit uses words. */
            /* The raw_readl function reads a word each time, so the reg address must be moved by 4 bytes. */
            reg += 4;
        }
    }
...
/* Note:there must be 100 clock cycles between two fifo operations.refer to bug1283 & bug1295 */
static int32_t can_fifo_write(can_t *can, uint32_t fifo_type, uint32_t *buf, uint32_t len)
{
	int32_t result = E_OK;
	uint32_t reg = 0;
	...
	if ( (NULL == can) ) {
		...
		switch (fifo_type) {
			case CAN_TX_FIFO:
				reg = REG_CAN_TX_FIFO_EL_OFFSET(0) + can->base;
				break;
			case CAN_TX_EVENT_FIFO:
				reg = REG_CAN_TX_EV_FIFO_EL_OFFSET(0) + can->base;
				break;
			default:
				result = E_PAR;
		}
...
static int32_t can_write_retry(can_t *can, uint32_t buf_id)
{
    int32_t result = E_OK;
...
static can_ops_t basic_can_ops = {
	...
	.timeout_counter_get = can_timeout_counter_get,
	.baud = can_baud_rate,
	...
	.tx_buf_add_request_status = can_tx_buffer_add_request_status,
	...
	.rx_lost_int_enabled = can_rx_lost_int_enabled,
	...
	.write_frame_try = can_write_retry,

	.version = can_version,
	.id_filter_element = can_id_filter_element_config,
	.rx_buf_size_get = can_rx_buf_size_get,
	.rx_buf_frame_num_get = can_rx_buf_frame_num_get
};
