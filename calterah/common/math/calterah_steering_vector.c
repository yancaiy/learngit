...
#define PRINT_DOA_ANT_INFO  false

static uint8_t print_flag = 0;
void reset_ant_pos_print_flag()
{
        print_flag = 0;
}
...
#if PRINT_DOA_ANT_INFO
                        if (print_flag == 0) {
                                EMBARC_PRINTF("ant_pos(%d) = %3.4f;", ch+1, ant_pos[ant_idx[ch]].x);
                                EMBARC_PRINTF(" ant_comps(%d) = %3.4f;\n", ch+1, ant_comps[idx]);
