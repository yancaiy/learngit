		...
		uint32_t *cfg_id, uint8_t *param, uint32_t len)
{
	...
	if ((NULL == dw_ssi) || (NULL == dw_ssi->ops) || \
		...
		*cfg_id = 0;
		...
		while (len) {
			dword1 = SFDP_PARAMETER_DWORD(param, idx);

			xfer.addr = SFDP_PARAMETER_DWORD(param, idx + 1);
		...
		uint8_t *param, uint32_t len)
{
	...
	if ((NULL == region_cnt) || (NULL == param) || (0 == len)) {
		...
		while (len) {
			region_header = SFDP_PARAMETER_DWORD(param, idx);
...
static int sfdp_map_descriptor_parse(flash_device_t *device, uint8_t *param, uint32_t len)
{
	...
	if ((NULL == device) || (NULL == param) || (len < 4)) {
		...
		param += 4;
		for (loopcnt = 0; loopcnt < region_cnt; loopcnt++) {
			dword1 = SFDP_PARAMETER_DWORD(param, loopcnt);
			device->m_region[loopcnt].size = dword1 >> SFDP_REGION_SIZE_SHIFT;
			device->m_region[loopcnt].base = base;
			base += device->m_region[loopcnt].size;

			erase_type_mask = dword1 & SFDP_REGION_ERASE_TYPE_MASK;
			device->m_region[loopcnt].erase_type_mask = erase_type_mask;
