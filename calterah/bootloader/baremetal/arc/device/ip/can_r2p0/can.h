...
#define CAN_INT_RX_FIFO_FILLED	(CAN_INT_RX_FIFO_REAL_FULL | CAN_INT_RX_FIFO_FULL) //| CAN_INT_TIMEOUT_OCCURED)
#define CAN_INT_TX_FIFO_READY	(CAN_INT_TX_FIFO_EMPTY | CAN_INT_TX_EVENT_FIFO_EMPTY)

#define MAX_CAN_FILTER_NUM (20)
...
typedef enum {
	...
	eSPEC_RX_BUF,
	eREJ_ID,
	eNUM_OF_CAN_STD_FILTER_ELEMENT_CFG
} eCAN_STD_FILTER_ELEMENT_CFG;

typedef struct can_id_filter_parameters {
	uint32_t frame_format;
	uint32_t filter_size;
	uint32_t mask;        /* bug1603,mask can only be 0x1fffffff */
	bool reject_no_match;
	//bool reject_remote; /* bug705,missing feature */
...
typedef enum can_timeout_op {
        eCAN_TIMEOUT_DISABLE = 0,
        eCAN_TIMEOUT_ENABLE  = 1
} can_timeout_op_t;

typedef enum can_timeout_sel {
        eCAN_TIMEOUT_CONTINUOUS_MODE = 0,
        eCAN_TIMEOUT_TX_EVENT_FIFO   = 1,
        eCAN_TIMEOUT_RX_FIFO         = 2
} can_timeout_sel_t;
...
typedef struct can_operation {
	...
	int32_t (*timeout_config)(can_t *can, can_timeout_op_t enable, uint32_t period, can_timeout_sel_t select);
	int32_t (*timeout_counter_get)(can_t *can, uint32_t *counter_value);
	...
	int32_t (*tx_buf_add_request_status)(can_t *can, uint32_t buf_id);
	...
	int32_t (*write_frame_try)(can_t *can, uint32_t buf_id);

	int32_t (*fifo_status)(can_t *can, uint32_t fifo_type, uint32_t fifo_sts);
	int32_t (*fifo_read)(can_t *can, uint32_t *buf, uint32_t len);
	int32_t (*fifo_write)(can_t *can, uint32_t fifo_type, uint32_t *buf, uint32_t len);
	...
	int32_t (*rx_lost_int_enabled)(can_t *can);
	...
	int32_t (*rx_buf_size_get)(can_t *can);
	int32_t (*rx_buf_frame_num_get)(can_t *can);
} can_ops_t;
