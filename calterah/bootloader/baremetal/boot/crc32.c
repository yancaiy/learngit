...
#ifdef BOOT_USE_HW_CRC
unsigned int hw_crc32_update(unsigned int crc, unsigned int *data, unsigned int len)
{
    unsigned int idx, single_len = 0xFFFF; //max 64K word
    ...
    while (len) {
        ...
        raw_writel(REL_REGBASE_CRC + REG_CRC_INTR_EN, 1);
        ...
        //udelay(1);
        while(!(INT_HW_CRC_COMPLETED(hw_crc_interrupt_status())));
        hw_crc_interrupt_clear(INT_HW_CRC_COMPLETE);
