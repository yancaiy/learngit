...
int32_t i2c_init(uint32_t id, i2c_params_t *params)
{
	...
	do {
		...
		/* if you want i2c running under high speed mode,need to config "params->restart_en == 1" */
		if (NULL != params) {
			runtime->ctrl.mode = 1;
			runtime->ctrl.speed = params->speed_mode;
			runtime->ctrl.addr_mode = params->addr_mode;
			runtime->ctrl.restart_en = params->restart_en;
			runtime->ctrl.rx_timeout = params->rx_timeout;
...
int32_t i2c_read(uint8_t *data, uint32_t len)
{
	...
	uint32_t disable = 0;
	i2c_runtime_t *runtime = &i2c_drv.runtime;
	uint64_t current_cycle = timer_rtc_count();
	/* set timeout cycle, since alps CPU is 300MHz, the following algorithm is calculated
	according to the timeout time whose unit is us */
	uint64_t timeout_cycle = (runtime->ctrl.rx_timeout * 300) + current_cycle;
	uint64_t runtime_cycle = 0;
	...
	do {
		...
		while (0 == DW_I2C_RXFIFO_N_EMPTY(dev_status)) {
			i2c_ops->status(i2c_dev, &dev_status);
			runtime_cycle = timer_rtc_count();
			if (runtime_cycle > timeout_cycle) {
				result = E_TMOUT;
				break;
			}
		}
		...
		if (result >= 0) {
			...
			EMBARC_PRINTF("error happened when i2c read len-1 bytes");
			break;
		   }
		}
		//read last byte
		...
		if (result >= 0) {
			...
			EMBARC_PRINTF("error happened when i2c read last byte");
			break;
		}
