...
ALL_ALLOBJS = $(ARC_OBJS) $(CHIP_OBJS) $(BOARD_OBJS) $(DEVICE_OBJS) $(EXT_DEV_OBJS) $(PROJECT_COBJS) $(BOOT_ASMOBJS) $(BOOT_COBJS) $(CAN_UDS_COBJS) $(CAN_UDS_BM_COBJS)
##
...
# include dependency files of application
 ...
 -include $(EXTRA_DEPS)
 -include $(ALL_DEPS)
 -include $(call get_deps, $(DEVICE_BAREMETAL_DRV_OBJS))
#endif
...
ALL_CSRC_DIRS += $(CAN_UDS_ROOT)
ALL_CSRC_DIRS += $(CAN_UDS_BM_ROOT)
...
bin : $(APPL_FULL_NAME).bin $(EXTRA_BIN_NAME) $(APPL_FULL_NAME).ini $(APPL_FULL_NAME).dump
...
LINK_OPT += -fno-builtin
endif

#####RULES FOR GENERATING ELF FILE#####
.SECONDEXPANSION:
$(APPL_FULL_NAME).elf : $(ALL_GENERATED_DIRS_TMPFILES) $(CORE_ARG_FILES) $(APPL_LINK_FILE) $(ARC_OBJS) $(CHIP_OBJS) $(BOARD_OBJS) $(DEVICE_OBJS) $(DEVICE_BAREMETAL_DRV_OBJS) $(EXT_DEV_OBJS) $(CAN_UDS_COBJS) $(CAN_UDS_BM_COBJS) $(PROJECT_COBJS) $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_LINK)
	$(Q)$(LD) $(LINK_OPT) $(ARC_OBJS) $(CHIP_OBJS) $(BOARD_OBJS) $(DEVICE_OBJS) $(DEVICE_BAREMETAL_DRV_OBJS) $(EXT_DEV_OBJS) $(CAN_UDS_COBJS) $(CAN_UDS_BM_COBJS) $(PROJECT_COBJS) -o $@
...
.SECONDEXPANSION:
$(CAN_UDS_COBJS) :$(OUT_DIR)/common/can_uds/%.o: %.c $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_COMPILE)
	$(Q)$(CC) -c $(COMPILE_OPT) $< -o $@

.SECONDEXPANSION:
$(CAN_UDS_BM_COBJS) :$(CAN_UDS_BM_OUT_DIR)/%.o: %.c $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_COMPILE)
	$(Q)$(CC) -c $(COMPILE_OPT) $< -o $@
...
$(APPL_FULL_NAME).ini : $(APPL_FULL_NAME).elf
	...
	$(Q)$(OBJDUMP) $(BIN_LOAD_OPT) $< | $(GREP) $(SEC_NOR_OTA) >> $@
