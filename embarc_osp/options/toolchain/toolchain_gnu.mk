...
TOOLCHAIN_DEFINES += $(HEAP_DEFINES) $(STACK_DEFINES) $(XIP_DEFINES) -D__GNU__
...
ifeq "$(HOST_OS)" "Msys"
	...
	src := $(shell for %%a in ("%path:;=" "%") do @echo %%~a | findstr /r "arc_gnu")
	obj := $(src:bin=share/openocd/scripts)
	OPENOCD_SCRIPT_ROOT = $(obj)
else
...
ifeq ($(WARNING_TO_ERROR),1)
	...
	COMPILE_OPT	+= $(CCORE_OPT_GNU)   $(ADT_COPT)   $(COMMON_COMPILE_OPT) -std=gnu99 -Wno-stringop-truncation -mlong-calls
...
#
#   additonal options for boot sequence.
#
	BIN_LOAD_OPT		= -h
	BIN_EXEC_OPT		= -d
	SEC_INIT		= ".init"
