...
extern int aes_decrypt(uint32_t *data_in, uint32_t *data_out, uint32_t len);

#define IMAGE_RAM_BUF_BASE	(0x770000)
...
static int32_t uart_ota_mem_clean(uint32_t payload_len);
static uint32_t uart_ota_update_image_handle(uint32_t sn, uint32_t type,  uint32_t payload_len, uint32_t faddr );
static int32_t uart_ota_send_ack(uint32_t ack);

void *ota_memcpy(uint8_t *dst, uint8_t *src, uint32_t len)
{
	while (len--) {
		*dst++ = *src++;
	}
...
/**
 * @brief flash w/e operation unlock via OTA host
 * 
 * @param nXORMagic : the key from OTA host,in /SOURCECODE/key.txt
 * @return uint8_t : unlock status
 *                  0 is OK
 *                  1 is MAGICID wrong
 *                  2 is CHECKSUM wrong
 */
static uint8_t process_ota_operations(unsigned int nXORMagic)
{
    /*start address of section .nor_ota*/
    extern unsigned int  _f_nor_ota;
    #define CAL_BOOT_NOR_OTA_XOR_MAGIC  (0x464f4243)

    unsigned int *pControl = &_f_nor_ota;
    unsigned int nMagicID = pControl[0];
    int nSize = pControl[1]>>2;
    unsigned int nCheckSumOrg = pControl[2];
    // unsigned int nXORMagic = pControl[3];
    int nIndex;
    unsigned int nCheckSumCal = 0;

    /*control flow need fine tune for robust boot*/
    /*support re-entry?*/
    if (nMagicID != CAL_BOOT_NOR_OTA_XOR_MAGIC)
        return 1;

    /*XOR the content of section .nor_ota*/
    for (nIndex = 4;nIndex < nSize;++nIndex) {
        pControl[nIndex] ^= nXORMagic;
        nCheckSumCal += pControl[nIndex];
    }

    if (nCheckSumCal == nCheckSumOrg) {
        /*flush D-cache*/
        dcache_flush();
        /*Invalide I-cache*/
        icache_invalidate();
        /*any action to prevent re-entry?*/
        //pControl[0] ^= 1;
    } else {
		return 2;
        /*break here?*/
...
/**
 * @brief uart ota handshake
 *        CAUTION! it's not standard flow,but can be work
 * 					it's highly coupled with framework
...
static void uart_ota_comm_handshake(void)
{
	...
	while (1) {
		...
		//is UART_OTA_COM_MAGIC_NUM(0x7856)?
		...
		{
			//is UART_OTA_START_STR2(0x756e)?
			if ((UART_OTA_START_STR2 == magic_num) && \
			    (UART_OTA_START_STR3 == hs_code)) {

				result = uart_write(hs_msg, len);

				//get key
				result = uart_read(hs_msg, 8);
				if (0 != result) {
					/* TODO: reset and reinit uart device. */
					continue;
				}
				
				nXORKey = DECRYPT_FLASH_KEY(hs_msg);
				result = process_ota_operations(nXORKey);
				for (size_t i = 4; i < 7; i++) {
					hs_msg[i] = 0xAA;
				}
				hs_msg[7] = (uint8_t)result;
				result = uart_write(hs_msg, len);
		...
		//STEP1: is UART_OTA_START_STR0(0x7561)?
		if ((UART_OTA_START_STR0 == magic_num) && (UART_OTA_START_STR1 == hs_code)) {
			result = uart_read(hs_msg, 2);
		}
	}

    // /*Temp solutioin for here, should get key from OTA tool and take decrypt and consistence check*/
    // process_ota_operations();
...
static int32_t uart_ota_mem_clean(uint32_t payload_len)
{
	...
	for (i = 0; i<payload_len; ++i){
		((unsigned char *)IMAGE_RAM_BUF_BASE)[i]=0;
	}
       return result;
}
...
static uint32_t uart_ota_update_image_handle(uint32_t sn, uint32_t type,  uint32_t payload_len, uint32_t faddr )
{
	...
	uint32_t ack = 0;
	uint32_t crc32 = 0, rx_crc32, verify_crc32 = 0;
	image_header_t old_header;

	uint32_t rx_len = payload_len + sizeof(uint32_t);
	uint8_t *payload_ptr = (uint8_t *)(ram_image_base);
	...
	do {
		...
		result = flash_memory_read(FLASH_FIRMWARE_BASE, \
				(uint8_t *)&old_header, \
		...
		if(!(faddr % FLASH_SECTOR_ERASE_SIZE)){
			result = flash_memory_erase(faddr, FLASH_SECTOR_ERASE_SIZE);
			if (0 != result) {
		...
		result = flash_memory_write(faddr, (const uint8_t *)IMAGE_RAM_BUF_BASE, payload_len);
		if (0 != result) {
			ack = ACK_NOR_FLASH_PROGRAM_FAILED;
			break;
		}

		result = uart_ota_mem_clean(payload_len);
		if (0 != result) {
			ack = ACK_MEM_CLEAN_FAILED;
			break;
		}

               /* verify data from external flash. */
		result = flash_memory_read(faddr, (uint8_t *)IMAGE_RAM_BUF_BASE, payload_len);
		if (0 != result) {
			ack = ACK_NOR_FLASH_READ_FAILED;
			break;
		}
		verify_crc32 = update_crc(0, (unsigned char *)IMAGE_RAM_BUF_BASE, payload_len);
		if (verify_crc32 != rx_crc32) {
			ack = ACK_COMM_CRC32_UNMATCHED;
			break;
		}
...
static uint32_t uart_ota_program_handle(uint32_t verify)
{
	...
	do {
        ...
        /*AES enabled for image header*/
        if (chip_security())
        {
            /*AES engine init*/
            aes_init();
            /*image header from flash decrypt*/
            aes_decrypt((void*)&old_header,(void*)&old_header,sizeof(old_header));
            ota_memcpy((void*)&img_header,(void*)header_ptr,sizeof(img_header));
            /*image header from PC decrypt*/
            aes_decrypt((void*)&img_header,(void*)&img_header,sizeof(img_header));
            header_ptr = &img_header;
        }

        /*magic check*/
		if ((0x43616c74UL != header_ptr->magic_number) ||
            (old_header.magic_number != header_ptr->magic_number)) {
			ack = ACK_NVM_MAGIC_NUM_UNMATCHED;
			break;
		}
		...
		if (0 != verify) {
			...
			/*use normal_boot(0) to verify the firmware_combine.bin*/
            /*to make OTA tool work well with former error code*/
			switch(normal_boot(0))
            {
            case E_CRC_MISMATCH: //CRC error from normal_boot will be transfer to ACK_NVM_CRC32_UNMATCHED
                ack = ACK_NVM_CRC32_UNMATCHED;
                break;
            case E_OK:
                ack = E_OK;
                break;
            default: //all other errors from normal_boot will be transfer to ACK_NOR_FLASH_PROGRAM_FAILED
                ack = ACK_NOR_FLASH_PROGRAM_FAILED;
                break;
            }
		}
...
void uart_ota_main(void)
{
	...
	uint8_t header[18] = {0};
	uint8_t cmd_id, sn, type;
	uint16_t payload_len = 0;
	uint32_t com_magic_num, rx_crc32, crc32, faddr = 0;
	...
	do {
		...
		while (1) {
			/* wait command:
			 ****************************************************************************
			 *  magic number  *  cmd_id  *  SN  *  length  *  faddr  *  type  *  CRC32  *
			 ****************************************************************************
			 * */
			...
			cmd_id = UART_OTA_CMD_ID(header);
			sn = UART_OTA_SN(header);
			payload_len = UART_OTA_PAYLOAD_LEN(header);
			faddr = UART_OTA_FADDR(header);
			/* type:ota file type 0-->header.bin 1-->boot.bin 2-->firmware_combine.bin*/
			type = UART_OTA_TYPE(header);
			rx_crc32 = UART_OTA_FRAME_CRC32(&header[14]);
			...
			if (0 == ack) {
				switch (cmd_id) {
					...
					case UART_OTA_PROGRAM_IMAGE:
						ack = uart_ota_program_handle(1);
						break;

					case UART_OTA_CMD_UPDATE_IMAGE:
						ack = uart_ota_update_image_handle(sn, type, payload_len, faddr);
						break;
