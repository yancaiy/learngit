...
static flash_device_t winbond_dev = {
	...
	.page_size = 256,
	.m_region = {
		DEF_FLASH_REGION(0x0, 0x1000000, 0x1000, 0x20)
	},
...
static int32_t winbond_quad_entry(dw_ssi_t *dw_ssi)
{
	...
	dw_ssi_xfer_t xfer = DW_SSI_XFER_INIT(CMD_WRITE_STS1_CFG2, 0, 8, 0);
	...
	do {
		if ((NULL == dw_ssi) || (NULL == dw_ssi->ops)) {
			...
			while ((E_OK == result) && (cnt--)) {
				chip_hw_mdelay(1);
				result = winbond_status_read(dw_ssi, FLASH_WEL);
			}
			if (!result) {
				result = E_TMOUT;
				break;
			...
			do {
				chip_hw_mdelay(1);
				result = winbond_status_read(dw_ssi, FLASH_WIP);
			} while ((E_OK != result) && (cnt--));
		}
	} while (0);
