...
#if ACC_BB_BOOTUP == 1
static bool reg_print = false;
#endif

static baseband_int_callback bb_done_callback = NULL;
static baseband_int_callback sam_done_callback = NULL;
extern EventGroupHandle_t bb_int_event_handle;

static sensor_config_t* bb_get_cfg(baseband_hw_t *bb_hw)
{
	sensor_config_t* cfg = (sensor_config_t*)CONTAINER_OF(bb_hw, baseband_t, bb_hw)->cfg;
	return cfg;
}

static fmcw_radio_t* bb_get_radio(baseband_hw_t *bb_hw)
{
...
{
        fmcw_radio_t* radio = bb_get_radio(bb_hw);
...
void baseband_interframe_power_save_enable(baseband_hw_t *bb_hw, bool enable)
{
        ...
        if(enable) {
                ...
                fmcw_radio_rx_on(radio, false);
                fmcw_radio_lo_on(radio, false);
                fmcw_radio_adc_ldo_on(radio, false);
        } else {
...
static void get_sample_used(baseband_hw_t *bb_hw, uint32_t * samp_skip_arg, uint32_t * samp_used_arg)
{
        sensor_config_t* cfg = bb_get_cfg(bb_hw);
        uint32_t samp_skip = ceil(cfg->adc_sample_start * cfg->adc_freq) / cfg->dec_factor;
        samp_skip = (samp_skip % 2 == 0) ? samp_skip : samp_skip + 1;

        uint32_t samp_end = ceil(cfg->adc_sample_end * cfg->adc_freq) / cfg->dec_factor;
...
static void baseband_sys_init(baseband_hw_t *bb_hw)
{
        ...
        uint32_t samp_skip;
        uint32_t samp_used;
        get_sample_used(bb_hw, &samp_skip, &samp_used);
                        ...
                        )
 {
        ...
        for (int l=0 ; l < AGC_LNA_GAIN_LEVEL; l++) {
		      ...
		      vga1_loop_end = false;
				...
				if((tot_lna_tia==gain_lna_tia) && (l>lna_idx)){
                                        lna_idx = l;
                                        tia_idx = t;
				}
                                }
...
static void baseband_agc_clear(baseband_hw_t *bb_hw) {
        ...
        /* below operation is must,to avoid generate err saturation status when reconfig AGC*/
        baseband_write_reg(bb_hw, BB_REG_AGC_IRQ_CLR, 0xfff);
        baseband_write_reg(bb_hw, BB_REG_AGC_IRQ_ENA, 0x0);
        baseband_write_reg(bb_hw, BB_REG_AGC_CHCK_ENA, 0x0);
...
static void baseband_fft_init(baseband_hw_t *bb_hw)
{
        ...
        BB_WRITE_REG(bb_hw, FFT_DBPM_ENA     , !( (cfg->anti_velamb_en) || VEL_DEAMB_MF_EN || CUSTOM_VEL_DEAMB_MF || DDM_EN) );
        ...
        if (cfg->phase_scramble_on) {
                ...
                for (int logic_idx = 0; logic_idx < cfg->nvarray; logic_idx++) {
                        ...
                        for (int phy_idx = 0; phy_idx < MAX_NUM_TX; phy_idx++) {
                                if (cfg->tx_groups[phy_idx] == (1 << (4 * logic_idx))) {
                                        ps_cpx_f.r = cos(cfg->phase_scramble_comp[phy_idx] * DEG2RAD)*(cfg->phase_scramble_comp_amplitude[phy_idx]);
                                        ps_cpx_f.i = sin(cfg->phase_scramble_comp[phy_idx] * DEG2RAD)*(cfg->phase_scramble_comp_amplitude[phy_idx]);

                                        if (cfg->anti_velamb_en) {
                                                if (logic_idx == 0) {
                                                        baseband_write_reg(bb_hw,
                                                                           (uint32_t)(((uint32_t *)BB_REG_FFT_DINT_COE_PS_0)),
                                                                           complex_to_cfx(&ps_cpx_f, 14, 1, true));
                                                        baseband_write_reg(bb_hw,
                                                                           (uint32_t)(((uint32_t *)BB_REG_FFT_DINT_COE_PS_0) + 1),
                                                                           complex_to_cfx(&ps_cpx_f, 14, 1, true));
                                                } else {
                                                        baseband_write_reg(bb_hw,
                                                                           (uint32_t)(((uint32_t *)BB_REG_FFT_DINT_COE_PS_0) + logic_idx + 1),
                                                                           complex_to_cfx(&ps_cpx_f, 14, 1, true));
                                                }
                                        } else {
                                                baseband_write_reg(bb_hw,
                                                                   (uint32_t)(((uint32_t *)BB_REG_FFT_DINT_COE_PS_0) + logic_idx),
                                                                   complex_to_cfx(&ps_cpx_f, 14, 1, true));
                                        }
                                }
                        }
...
void baseband_velamb_cd_init(baseband_hw_t *bb_hw)
{
        ...
        memset(&n2d_info, 0, sizeof(n2d_info));
        n2d_info.Tr_2_PI_Over_TDN = 2.0 * M_PI * Tr / (Td * nfft2);
        n2d_info.ph_comp_flg = ((cfg->anti_velamb_en) || VEL_DEAMB_MF_EN || CUSTOM_VEL_DEAMB_MF || DDM_EN);
...
void read_steering_vec_from_mem(baseband_hw_t *bb_hw, int num_chan, uint32_t addr_coe, uint32_t d, complex_t *vec)
{
        ...
        for (loop = 0; loop < (num_chan/MAX_NUM_RX); loop++){
                for (ch = 0; ch < MAX_NUM_RX; ch++) {
                        uint32_t fix_val = baseband_read_mem_table(bb_hw, addr_coe + d * mem_size + loop * MAX_NUM_RX + ch);
                        complex_t sv_elem = cfx_to_complex(fix_val, 14, 1, true);
                        /* the steering vector stored in mem is conjugated version of real sv */
                        vec[loop * MAX_NUM_RX + ch] = cconj(&sv_elem);
                }
        }
...
static void gen_dml_dk(baseband_hw_t *bb_hw, int g, double delta, double delta2, antenna_pos_t * ant_pos, uint8_t sv_size, uint8_t * ant_idx) {
        sensor_config_t* cfg = bb_get_cfg(bb_hw);
...
{
        int32_t status;
        uint32_t buff[32]; // maximal 32 words
        int mem_size = ((doa_ant_num + 3)/4)*4;  //number of words per angle
        uint32_t length = mem_size * BYTES_PER_WORD;
        int d = 0;
        for (d = 0; d < angle_num; d++) {
                status = flash_memory_read(read_addr + d * length, (uint8_t *)buff, length);  //length: number of bytes
                if (status != E_OK) {
...
static void baseband_doa_init(baseband_hw_t *bb_hw)
{
        ...
        if (cfg->doa_mode == 1) {
                ...
                n2d_info.azi_line_num = comb_size_v;
                for (int g = 0; g < comb_size_v; g++) {
                        n2d_info.azi_vector_size[g] = get_sum_idx_from_mux(cfg->combined_doa_fft_mux[g],
                                                                           n2d_info.azi_logic_channel_idx[g]);
                }
        }
#if (RNE_ENA == 1)
        else if (cfg->doa_mode == 0) {
                // RNE function add begin ,get n2d_info value in normal mode
                // normal mode only cal main array pow1_c and Pow2_c
                n2d_info.azi_line_num = 1;
                n2d_info.azi_vector_size[0]  = get_sum_idx_from_mux(cfg->doa_fft_mux[0], n2d_info.azi_logic_channel_idx[0]);
        ...
        BB_WRITE_REG(bb_hw, DOA_C2D1_SIZ_CMB, combined_2d_comb_num[1] - 1);    /* combined_2d_comb_num: combined mode */
                ...
                if (cfg->sv_read_from_flash == false) {
                        ...
                        if ((cfg->doa_mode == 1) && (g == 1)) {
                                ...
                                for (int ele_rx = 0; ele_rx < sv_size; ele_rx++) {
                                        ...
                                        n2d_info.vertical_dis[ele_rx] = ant_pos[ant_idx[ele_rx]].y;
                                }
                        }
                        ...
                        gen_dml_dk(bb_hw, g, delta, delta2, ant_pos, sv_size, ant_idx);
                } else {
        ...
        if (cfg->doa_mode == 1) {
                ...
                for (int g = 1; g < comb_size_v; g++) {    //g = 0 is group 0 and it has been configured
                        ...
                        n2d_info.azi_sv_add[g] = c2d_addr_coe[g - 1];
                }
                ...
                //RNE function add begin
                // get normal mode sv add, and there are only have main array in this mode
                n2d_info.azi_sv_add[0] = doa_grp_addr_coe[0]; // whether sv_read_from_flash is false ,this add will not change
                //RNE function add END
        }
#endif
        baseband_switch_mem_access(bb_hw, old);
...
static void baseband_debpm_init(baseband_hw_t *bb_hw)
{
        ...
        uint32_t offset = CFAR_MIMO_COMB_COE + DEBPM_COE/MAX_FRAME_TYPE*bb_hw->frame_type_id; //space for mimo combine coe
...
static void baseband_dc_calib_calc(baseband_hw_t *bb_hw, int16_t dc_offset[ANT_NUM], bool radio_cfg_ena, bool print_ena) /* leakage enalbe */
{
        ...
        /* Disable dmu irq for polling mode */
        dmu_irq_enable(INT_BB_SAM, 1);
        dmu_irq_enable(INT_BB_DONE, 0);
        baseband_hw_start_with_params(bb_hw, bb_status_en, BB_IRQ_ENABLE_ALL); /* IRQ SAM_DONE, for power saving handler*/
...
#if defined(FUNC_SAFETY) && ((SAFETY_FEATURE_SM104 == FEATURE_ON) || (SAFETY_FEATURE_SM105 == FEATURE_ON))
        clear_all_bb_memory(bb_hw);
#endif
...
{
        dmu_irq_enable(INT_BB_SAM, sys_irq_en & 0x4);
        dmu_irq_enable(INT_BB_DONE, sys_irq_en & 0x1);
        baseband_hw_start_with_params(bb_hw, sys_enable, BB_IRQ_ENABLE_ALL);
        BASEBAND_ASSERT(baseband_wait_bb_done(POL_MODE, DEFAULT_TIMOUT) == E_OK);
}
...
void Txbf_bb_satur_monitor(baseband_hw_t *bb_hw, unsigned int tx_mux)
{
        ...
        for (loop = 0; loop < 8; loop++) {
                ...
                baseband_start_with_params(cfg->bb, true, false,
                                   bb_status_en,
                                   false, BB_IRQ_ENABLE_BB_DONE, false);
                /* wait done */
                baseband_wait_bb_done(INT_MODE, DEFAULT_TIMOUT);
...
void baseband_reg_dump(baseband_hw_t *bb_hw)
{
        ...
        for(int idx = 0; idx < sizeof(bb_reglist) / sizeof(uint32_t); idx++) {
            ...
            EMBARC_PRINTF("0x%x  0x%08x\n\r", bb_reglist[idx], baseband_read_reg(bb_hw, bb_reglist[idx]));
        }
}
...
{
        uint32_t old_irq_mask = raw_readl(REL_REGBASE_DMU + REG_DMU_IRQ_ENA32_63_OFFSET);
        uint32_t bb_done_irq_enabled = old_irq_mask & (0x00000001 << 0);
        uint32_t bb_sam_done_irq_enabled = old_irq_mask & (0x00000001 << 2);
...
void baseband_dac_playback(baseband_hw_t *bb_hw, bool inner_circle, uint8_t inject_num, uint8_t out_num, bool adc_dbg_en, float* peak_power)
{
        ...
        uint32_t old_bnk_act = baseband_switch_bnk_act(bb_hw, 0x0);
        uint32_t old_sys_type_fmcw = BB_READ_REG(bb_hw, SYS_TYPE_FMCW);
        BB_WRITE_REG(bb_hw, SYS_TYPE_FMCW, 0x0);

        /* switch to baseband shadow bank */
        baseband_switch_bnk_act(bb_hw, SHADOW_BNK);
        baseband_frame_interleave_pattern(bb_hw, 0x4);
        ...
        fmcw_radio_rx_on(radio, true);
#endif
        fmcw_radio_sdm_reset(radio);
        // UDELAY(100);//migt not needed
        ...
        baseband_start_with_params(cfg->bb, false, false, bb_status_en, false, BB_IRQ_DISABLE_ALL, false);
        ...
        baseband_switch_bnk_act(bb_hw, 0x0);
...
void baseband_hil_state_ctrl(baseband_hw_t *bb_hw, uint16_t bb_ena_0, uint16_t bb_ena_1, uint16_t bb_ena_2)
{
        ...
        dmu_irq_enable(INT_BB_SAM, 0);
        dmu_irq_enable(INT_BB_DONE, 0);
        baseband_hw_start_with_params(bb_hw, bb_ena_0, BB_IRQ_ENABLE_ALL);
        dbgbus_input_config();
...
void baseband_hil_on_ahb(baseband_hw_t *bb_hw)
{
        ...
        dmu_irq_enable(INT_BB_SAM, 0);
        dmu_irq_enable(INT_BB_DONE, 1);
        baseband_hw_start_with_params(bb_hw, bb_status_en, BB_IRQ_ENABLE_ALL);
...
void baseband_agc_dbg_reg_dump(baseband_hw_t *bb_hw, int item)
{
        ...
        case 3:
                ...
                break;
        default:
	       EMBARC_PRINTF("item not support.\n");
                break;
    }
}
...
/**
 * @brief Polling irq_status bit2 for sam_done
 *
 * @param timeout Polling for at most <timeout> ms
 * @return Returns E_OK if irq_status bit 2 set within timeout, else return E_TMOUT
 */
int32_t baseband_polling_sam_done(uint32_t timeout)
{
        int32_t ret = E_OK;
        while(!(BB_READ_REG(NULL, SYS_IRQ_STATUS) & 0x4 ) && timeout )
        {
                UDELAY(1000);
                timeout--;
        }
...
/**
 * @brief Polling irq_status bit0 for bb_done
 *
 * @param timeout Polling for at most <timeout> ms
 * @return Returns E_OK if irq_status bit0 set within timeout, else return E_TMOUT
 */
int32_t baseband_polling_bb_done(uint32_t timeout)
{
        int32_t ret = E_OK;
        while(!(BB_READ_REG(NULL, SYS_IRQ_STATUS) & 0x1) && timeout )
        {
                UDELAY(1000);
                timeout--;
        }
...
/**
 * @brief Set FreeRTOS Event bit2 to 1 for sam_done
 *
 * @param timeout Wait for event bit for at most <timeout> ms
 * @return Returns E_OK if irq_status bit2 set within timeout, else return E_TMOUT
 */
int32_t baseband_wait_bb_sam_event(uint32_t timeout)
{
        int32_t ret = E_OK;
        EventBits_t uxBits;
        uxBits = xEventGroupWaitBits(bb_int_event_handle, EVENT_SAM_DONE, pdTRUE, pdFALSE, pdMS_TO_TICKS(timeout));
        if((uxBits & EVENT_SAM_DONE) != EVENT_SAM_DONE)
        {
                ret = E_TMOUT;
        }
...
/**
 * @brief Set FreeRTOS Event bit0 to 1 for bb_done
 *
 * @param timeout Wait for event bit for at most <timeout> ms
 * @return Returns E_OK if irq_status bit0 set within timeout, else return E_TMOUT
 */
int32_t baseband_wait_bb_done_event(uint32_t timeout)
{
        int32_t ret = E_OK;
        EventBits_t uxBits;
        uxBits = xEventGroupWaitBits(bb_int_event_handle, EVENT_BB_DONE, pdTRUE, pdFALSE, pdMS_TO_TICKS(timeout));
        if((uxBits & EVENT_BB_DONE) != EVENT_BB_DONE)
        {
                ret = E_TMOUT;
        }
...
/**
 * @brief Baseband sample done interrupt service routine. Clear baseband SAM_DONE_IRQ.
 * Turn on power-saving mode. Do FuSa sm1 if enabled. Call sam_done_callback to set FreeRTOS Event bits.
 */
void baseband_sam_done_isr()
{
        ...
        baseband_interframe_power_save_enable(bb_hw, true);
#endif
#if defined(FUNC_SAFETY) && (SAFETY_FEATURE_SM1 == FEATURE_ON)
        /*set sm1(ldo monitor) parameter*/
        // func_safety_sm_ldo_monitor_set_part(&bb->radio);
        fusa_run_periodic_item(FUNC_SAFETY_ITEM_SM1, FUNC_SAFETY_ITEM_SM1_SET_PART);
        // log_fusa("[baseband_sam_done_isr]**** time =%f\n",fusa_get_current_time_ms());
#endif
        if(sam_done_callback != NULL)
        {
                sam_done_callback();
        }
}
...
/**
 * @brief Baseband done interrupt service routine. Clear baseband BB_DONE_IRQ.
 * Call sam_done_callback to set FreeRTOS Event bits.
 */
void baseband_bb_done_isr()
{
        /* Clear bb_done irq */
        BB_WRITE_REG(NULL, SYS_IRQ_CLR, BB_IRQ_CLEAR_BB_DONE);

        if (bb_done_callback != NULL)
        {
                bb_done_callback();
        }
}
...
/**
 * @brief Install sam_done_callback and bb_done_callback functions
 *
 * @param type  Callback type to be installed. SAM_DONE or BB_DONE
 * @param func Callback function to be installed
 * @return Return E_OK if either SAM_DONE or BB_DONE callback intstalled. Return E_PAR if parameter error.
 */
int32_t baseband_int_callback_install(baseband_int_type_t type, baseband_int_callback func)
{
        ...
        if(type == SAM_DONE)
        {
                sam_done_callback = func;
        } else if (type == BB_DONE)
        {
                bb_done_callback = func;
        } else {
                ret = E_PAR;
        }
        return ret;
...
/**
 * @brief SAM_DONE callback to set FreeRTOS Event bit2 to 1
 */
void baseband_sam_done_cb()
{
        xEventGroupSetBitsFromISR(bb_int_event_handle, EVENT_SAM_DONE, NULL);
}

/**
 * @brief BB_DONE callback to set FreeRTOS Event bit0 to 1
 */
void baseband_bb_done_cb()
{
        xEventGroupSetBitsFromISR(bb_int_event_handle, EVENT_BB_DONE, NULL);
}

/**
 * @brief SAM_DONE and BB_DONE interrupt init. Install int handler, enable CPU interrupt,
 * enable DMU interrupt, install callback function.
 */
void baseband_interrupt_init()
{
        /* SAM DONE ISR */
        int_handler_install(INT_BB_SAM, baseband_sam_done_isr);
        int_enable(INT_BB_SAM);
        dmu_irq_enable(INT_BB_SAM, 1);
        baseband_int_callback_install(SAM_DONE, baseband_sam_done_cb);

        /* BB DONE ISR */
        int_handler_install(INT_BB_DONE, baseband_bb_done_isr);
        int_enable(INT_BB_DONE);
        dmu_irq_enable(INT_BB_DONE, 1);
        baseband_int_callback_install(BB_DONE, baseband_bb_done_cb);
}

/**
 * @brief Wait sam_done in either INT_MODE or POL_MODE with timeout
 *
 * @param wait_mode Wait baseband sample done using interrupt mode or polling mode
 * @param timeout Wait at most <timeout> ms for either register or FreeRTOS Event
 * @return Returns E_OK if received Event or set register within timeout, else return E_TMOUT
 */
int32_t baseband_wait_sam_done(bb_wait_mod_t wait_mode, uint32_t timeout)
{
        int32_t ret = E_OK;
        if(wait_mode == INT_MODE)
        {
                ret = baseband_wait_bb_sam_event(timeout);
        } else {
                ret = baseband_polling_sam_done(timeout);
        }
...
/**
 * @brief Wait bb_done in either INT_MODE or POL_MODE with timeout
 *
 * @param wait_mode Wait baseband done using interrupt mode or polling mode
 * @param timeout Wait at most <timeout> ms for either register or FreeRTOS Event
 * @return Returns E_OK if received Event or set register within timeout, else return E_TMOUT
 */
int32_t baseband_wait_bb_done(bb_wait_mod_t wait_mode, uint32_t timeout)
{
        ...
        if(wait_mode == INT_MODE)
        {
                ret = baseband_wait_bb_done_event(timeout);
        } else {
                ret = baseband_polling_bb_done(timeout);
        }
...
/**
 * @brief Clear FreeRTOS Event bits
 *
 * @param event_bits FreeRTOS Event bits to be clear. Suggest to be called before baseband start.
 * @return Return 0 if no event bits are set before clear, else return bits set before clear.
 */
uint32_t baseband_clear_event(uint32_t event_bits)
{
        uint32_t ret = 0;
        uint32_t uxBits;
        uxBits = xEventGroupClearBits(bb_int_event_handle, event_bits);
        if(xEventGroupClearBits(bb_int_event_handle, event_bits) != 0){
                ret = uxBits;
        }
        return ret;
}