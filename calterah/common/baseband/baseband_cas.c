...
static uint32_t slv_head = 0;
static uint32_t slv_tail = 0;

void mst_slv_fram_cnt_chek()
{
        if ((slv_head != frame_cnt) || (slv_tail != frame_cnt + 2)) {
                while(1){
                        EMBARC_PRINTF("\n frame_cnt = %d, slv_head = %d, slv_tail = %d !!!! \n\n", frame_cnt, slv_head, slv_tail);
                }
        }
...
static int32_t baseband_merge_cfar(baseband_hw_t *bb_hw, uint8_t ant_chl_num, uint16_t mst_obj_num, uint16_t *slv_obj_num, uint16_t *rx_obj_num, uint32_t *mem_mac_offset, uint32_t *merge_num, cfr_info_t *cfr_info, volatile obj_info_t *obj_info)
{
        ...
        if (E_OK == cascade_read_buf_req(WAIT_TICK_NUM)) { /* wait 30ms */
                slv_head = cascade_read_buf(0); // read slave head for debug
                /* get slave object info */
                uint32_t buf_data = cascade_read_buf(BUF_SHIFT_INFO + SLV_HEAD_SIZ);     /* read cfar info of slave chip */
                *slv_obj_num = REG_H16(buf_data);
                *rx_obj_num = REG_L16(buf_data); // transmitted object number of slave

                rx_buf_offset = BUF_SIZE_INFO + SLV_HEAD_SIZ; // in case '*rx_obj_num == 0'
                /* merge */
                for (uint16_t i = 0; i < *rx_obj_num; i++) {
                        /* read slave: range, doppler */
                        rx_buf_offset = BUF_SIZE_INFO + i * (ant_chl_num + BUF_SIZE_CAFR) + SLV_HEAD_SIZ;
                ...
                slv_tail = cascade_read_buf(rx_buf_offset);
...
void baseband_write_cascade_ctrl(void)
{
        ...
        do {
                ...
                slv_head = frame_cnt;
                cascade_write_buf(slv_head); // write frame counter for debug
                ...
                // tx done
                slv_tail = frame_cnt + 2;
                cascade_write_buf(slv_tail);
