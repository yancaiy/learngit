...
static void cs_transmitter_init(void)
{
	...
	rt_cs_transmitter.state = SC_IDLE;
...
static void cs_isr_callback(void *params)
{
	...
	if (rt_cs_transmitter.state != SC_IDLE) {
...
int32_t console_wait_newline(uint8_t *data, uint32_t wbuf_len)
{
	...
	if (pdTRUE == xQueueReceive(queue_ptr, &msg, uart_cmd_time)) {
		// should not be 0, add one tick to disturb the idle task loop   portMAX_DELAY -> 0 -> 1 ->?
		uint32_t buf_tail;
...
static void post_printf(int32_t buf_id, uint32_t len)
{
	if (dbg_printf_flag) {
		...
		if ((re_en_flg) && (SC_IDLE == rt_cs_transmitter.state)) {
			cs_txbuf_install();
...
static void cs_txbuf_install(void)
{
	...
	if (re_install) {
		...
		if (E_OK == result) {
			...
			rt_cs_transmitter.state = SC_BUSY;
		} else {
...
void bprintf(uint32_t *data, uint32_t len, void (*tx_end_callback)(void))
{
	...
	/* avoid re-enter. */
	cpu_sts = arc_lock_save();
	if (SC_IDLE == rt_cs_transmitter.state) {
		cs_txbuf_install();
