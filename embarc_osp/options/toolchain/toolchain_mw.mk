...
ifdef LOAD_XIP_TEXT_EN
	XIP_DEFINES := -D_LOAD_XIP_TEXT=$(strip $(LOAD_XIP_TEXT_EN))
endif
...
TOOLCHAIN_DEFINES += $(HEAP_DEFINES) $(STACK_DEFINES) $(XIP_DEFINES) -D__MW__
...
##
#   For compliance between Calterah Dedicated MW tool and common license-required MW tool.
#   - Thers are "Alps Rhine" string in Revision info(ccac -version), if Calterah dedicated MW tool.
#   - Thers are NOT "Alps Rhine" string in Revision info(ccac -version), if common license-required MW tool.
##
MW_TOOL_VERSION=$(strip $(shell ccac --version | findstr Revision))
ifneq "$(findstring Alps Rhine, $(MW_TOOL_VERSION))" ""
	# Need below compile/link options, if Calterah Dedicated MW tool used.
	TCF_ASSIGN  = -tcf=AlpsRhineEM6
	OPTION_P_KEEP =
else
	# Need below compile/link options, if common license-required MW tool used.
	TCF_ASSIGN  =
	OPTION_P_KEEP = -P
endif
...
##
	...
	COMMON_COMPILE_OPT = -Hnoccm -Hnosdata -Wincompatible-pointer-types -Hnocopyr \
				$(OPT_OLEVEL) $(CDEBUG_OPTION) $(ALL_DEFINES) $(ALL_INCLUDES) $(MKDEP_OPT) $(TCF_ASSIGN)
	...
	LINK_OPT	+= $(LCORE_OPT_MW)   $(ADT_LOPT)   \
				-Hnocopyr -Hnosdata -Hnocrt $(LMAP_OPTION) $(APPL_LINK_FILE) $(TCF_ASSIGN)

ifeq ($(WARNING_TO_ERROR),1)
	COMMON_COMPILE_OPT += -Werror
endif


	## Link File Generation Options
	LINK_FILE_OPT	+= -Hnocopyr -Hasmcpp -Hasext=ld,met -Hnoarcexlib -Hnoarcsimd -E -nostdsysteminc -undef -D__MW__ $(ALL_INCLUDES) $(OPTION_P_KEEP)
	LINK_FILE_DEPOPT = $(LINK_FILE_OPT) -DENABLE_GENERATE_DEPENDCY_FILE $(MKDEP_OPT)

	## Other Options
	AR_OPT		+= -rsq
	TRACKLIB_AR_OPT += $(AR_OPT)
	DMP_OPT		+= -hIpst -z -S
	DASM_OPT	+= -z
	SIZE_OPT	+= -gq
...
##
	...
	BIN_LOAD_OPT		= -s
	BIN_EXEC_OPT		= -t
	SEC_INIT		= ".init_bootstrap"
	SEC_DATA		= ".data"
	SEC_HEADER		= ".nvm_header"
	EXEC_ENTRY		= "_arc_reset"
