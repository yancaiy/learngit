...
#ifndef CHIP_CASCADE
sensor_config_t config[MAX_FRAME_TYPE] =
{
#if NUM_VARRAY == 1
        {
...
static sensor_config_t config[MAX_FRAME_TYPE] =
{
        ...
        {
        #include "sensor_config_init2_cas.hxx"
                .bb = NULL,
        },
        {
        #include "sensor_config_init3_cas.hxx"
                .bb = NULL,
        },
};
#endif // CHIP_CASCADE
...
void sensor_config_preattach(sensor_config_t *cfg)
{
        int32_t status;
        uint32_t *head;
        ...
        */
        ...
        if (cfg->doa_mode == 1) {
                ...
                for (int line = 0; line < 4; line++) {
                        /* maximum 4 lines for horizontal direction */
                        int x_num = bit1num(cfg->combined_doa_fft_mux[line]);
                        x_num = ((x_num + 3) >> 2) << 2;
                        x_ch_num = x_ch_num + x_num;
                        if (x_num)
                        {
                            y_ch_num++;
                        }
                }
                y_ch_num = ((y_ch_num + 3) >> 2) << 2;

                int coe_mem_sz = x_ch_num * cfg->doa_npoint[0] + y_ch_num * cfg->doa_npoint[1];
                EMBARC_ASSERT(coe_mem_sz <= 360 * 32);
        }
        ...
        if (cfg->doa_method == 2) {
                /* dml */
                EMBARC_ASSERT(cfg->dml_2dsch_end[0] < cfg->doa_npoint[0]);

                if (cfg->doa_mode == 1) {
                        EMBARC_ASSERT(cfg->dml_2dsch_end[1] < cfg->doa_npoint[1]);
                        EMBARC_ASSERT((cfg->doa_npoint[0] + cfg->doa_npoint[1]) <= 1024);
                }

                /* work around for bug 1045; prevent below configuration */
                if (cfg->doa_mode == 1) {
                        /* combined mode */
                        int32_t start_A = (int32_t)cfg->dml_2dsch_start[0];
                        int32_t start_B = (int32_t)cfg->dml_2dsch_start[1];

                        int32_t step_A = (int32_t)cfg->dml_2dsch_step[0];
                        int32_t step_B = (int32_t)cfg->dml_2dsch_step[1];

                        int32_t end_A = (int32_t)cfg->dml_2dsch_end[0];
                        int32_t end_B = (int32_t)cfg->dml_2dsch_end[1];
                        ...
                        int32_t range_s = start_B - (step_B >> 1);
                        range_s = MAX(0, range_s);
                        EMBARC_ASSERT(range_s != end_A);

                        while (range_s <= end_A) {
                                start_B = start_B + step_B;
                                range_s = start_B - (step_B >> 1);
